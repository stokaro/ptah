...
// schema (Database and its schemas, tables, columns, indexes, and constraints)
// shared by the dbschema readers and writers and consumed by schema diffing.
//
// The type names carry no DB prefix: the package name already says which side
// of the pipeline a value came from, so catalog.Table is the live table and
// schemamodel.Table is the declared one.
//
// Struct field names are deliberately left alone. Every type here is
// serialized -- schema artifacts, plan documents, and the agent surface all
// marshal these structs -- so a field rename is a wire-format change with
// on-disk consequences, not a naming question. Renaming the types costs
// nothing on the wire because the JSON tags name the fields, not the types.
package catalog

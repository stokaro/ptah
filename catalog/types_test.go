...
func TestDatabase_RolesOutOfScopeIsNeverSerialized(t *testing.T) {
	c := qt.New(t)
	...
	// golden snapshots. Both serialize a Database through encoding/json, so a
...
func TestConstraint_ColumnSlicesFallbackToLegacyFields(t *testing.T) {
	c := qt.New(t)

...
func migrateUpCommand(cmd *cobra.Command, opts *options) error {
	...
	if err := requireLockTimeoutForOnlineMode(lintPolicy, conn.Info().Dialect, timeouts); err != nil {
		return err
	}
...
//
// The transaction mode does not change the answer. A migration that runs
// outside a transaction takes the timeout on its session, and such a file is
// where the online form of a constraint lives: `ADD CONSTRAINT ... NOT VALID`
// takes ACCESS EXCLUSIVE for an instant like any other ALTER, and it is marked
// no_transaction only so the VALIDATE after it scans under the weaker lock
// (stokaro/ptah#3501).
//

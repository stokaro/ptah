...
//
// It lives inside the command tree rather than beside the shared package
// because `internal/cli/internal/editor` owns one of the variables, and Go's
// internal-import rule lets only packages under `internal/cli/` import it. A
// guard that could not see one owner's registrations would report that
// owner's variables as unclassified.
//

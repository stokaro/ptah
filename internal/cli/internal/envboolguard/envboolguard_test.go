...
import (
	...
	_ "ptah.run/internal/cli/internal/editor" // links its PTAH_* declarations into the registry
...
var nonBooleanPtahVars = []string{
	// The pinned Atlas community binary the conformance tests measure against.
	...
	"PTAH_MIGRATIONS_ENGINE", // storage engine for the revision table (ClickHouse)
...
`,
		...
		},
		{
			...
			// one internal/cli/internal/cmdflags actually had: the read and the parse are
			// two functions apart.
...
func scanFile(c *qt.C, root, path string) []string {
	...
	// function-level rule: internal/cli/internal/cmdflags read the value in `applyEnv`
...
func goSourceFiles(c *qt.C, root string) []string {
	...
	for line := range strings.SplitSeq(strings.TrimSpace(out.String()), "\n") {
		...
		if strings.HasPrefix(line, "internal/envbool/") || strings.HasPrefix(line, "internal/cli/internal/envboolguard/") {
			continue
		}

...
func TestMigrateHashRejectsExplicitEmptyDirectoryWithoutWritingSum(t *testing.T) {
	...
	// `migrate hash --dir=` exits 1 with this line, where reaching the stat
	// reports `migrations directory : stat : no such file or directory`
	// instead (stokaro/ptah#1186). What the test is really about is below: nothing is
	// written either way.

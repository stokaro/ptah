...
func TestConvertApplySourceReportsSourceParseFailures(t *testing.T) {
	c := qt.New(t)
	// A Down section with no Up before it. A file with no directives at all
	// cannot serve here: Atlas executes such a file verbatim and so does Ptah
	// (stokaro/ptah#981), so it says nothing about where parsing happens. A
	// broken directive ORDER cannot be read, which is what this test needs.
	dir := writeSplitFixture(c, map[string]string{

...
func mysqlConn() fakeConn {
	// #nosec -- fixture URL with a made-up password, not a credential
...
func requireCredentialFileRemoved(t *testing.T, path string) {
	t.Helper()
	// #nosec -- test helper verifies cleanup of a path created by the code under test
...
func TestMySQLDefaultsFileContent(t *testing.T) {
	...
	// #nosec -- fixture password exercises escaping, not a credential

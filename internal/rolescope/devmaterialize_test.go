...
func roleNames(roles []schemamodel.Role) []string {
	var names []string
...
func TestRolesToCreateOnDevSkipsWhatTheServerAlreadyHas(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			declared:         []schemamodel.Role{{Name: "ptah_user"}, {Name: "app_reader"}},
		...
		},
		{
			...
			declared:         []schemamodel.Role{{Name: "brand_new"}},
		...
		},
		{
			...
			declared:         []schemamodel.Role{{Name: "app_reader"}, {Name: "brand_new"}},
		...
		},
		{
			...
			declared:         []schemamodel.Role{{Name: "app_reader"}, {Name: "app_writer"}},
			present:          nil,
		...
		},
		{
			...
			declared:         []schemamodel.Role{{Name: "App_Reader"}},
			present:          []catalog.Role{{Name: "app_reader"}},
...
func TestRolesToCreateOnDevKeepsTheDeclaredAttributes(t *testing.T) {
	...
	c.Assert(alreadyThere, qt.DeepEquals, []schemamodel.Role{{Name: "app_reader", Login: true, CreateDB: true}})
	c.Assert(create, qt.DeepEquals, []schemamodel.Role{{Name: "brand_new", Superuser: true}})
}
...
func TestReportNotCreatedOnDevNamesEveryRoleItSkipped(t *testing.T) {
	tests := []struct {
		name            string
		alreadyOnServer []schemamodel.Role
		want            string
	}{
		{
			name:            "one role is singular",
			alreadyOnServer: []schemamodel.Role{{Name: "app_reader"}},
		...
		},
		{
			...
			alreadyOnServer: []schemamodel.Role{{Name: "ptah_user"}, {Name: "app_reader"}},
...
func TestReportNotCreatedOnDevStaysSilentWhenItCreatedEverything(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			report: func(out *bytes.Buffer) {
				rolescope.ReportNotCreatedOnDev(out, make([]schemamodel.Role, 0))
			},
		},
		{
			name: "a nil writer takes the note with it",
			report: func(_ *bytes.Buffer) {
				rolescope.ReportNotCreatedOnDev(nil, []schemamodel.Role{{Name: "app_reader"}})
			},
		},
	}

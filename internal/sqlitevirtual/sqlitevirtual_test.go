...
func TestValidateComparison(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			current := &catalog.Database{Tables: tt.database}

			err := sqlitevirtual.ValidateComparison(tt.dialect, tt.desired, current, tt.policy)
...
func TestTables(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Assert(sqlitevirtual.Tables(tt.current), qt.DeepEquals, tt.want)
		})
	}
}

...
func readColumns(
	...
	if len(desired) == 0 {
		if want == countRows {
			return managedColumns(nil, md.Keys), desired, nil
		}
		if liveTable == nil {
	...
	columns := managedColumns(desired, md.Keys)
	if liveTable == nil {
	...
	narrowed := projectOntoLive(columns, *liveTable)
...
// the read asks only for what the database can return.
//
// A column the declaration names and the table has not gained yet is structural
// drift, and the structural comparison is what reports it. The row comparison
// then covers the columns both sides carry, which is every column it can say
// anything about: the live row holds no value for the missing one, so no live
// value is at stake and reporting each row as an overwrite would misstate what
// applying costs.
//
// Reading the column instead is what one dialect does with a quoted name it
// cannot resolve: SQLite returns the name as a string literal, so every row
// differs from a value nothing in the database holds, while PostgreSQL answers
// 42703 and takes the whole check down.
...
func insertableColumns(dialect, qualified string, dbTable catalog.Table, keys []string) ([]string, error) {
...
func rejectsExplicitInsert(dialect string, col catalog.Column) bool {
...
func findManagedTable(dbSchema *catalog.Database, wantSchema, defaultSchema, table string) (catalog.Table, bool) {

...
// init emits a fixed SQL schema when this test binary is re-executed as an
// external schema command. Exiting here prevents the testing package from
// appending its own output to the schema document.
func init() { //nolint:gochecknoinits // A subprocess test fixture must run before the testing package.
...
func schemaCommand() schemasource.Command {
	return schemasource.Command{
		Args: []string{os.Args[0], "-test.run=^$"},

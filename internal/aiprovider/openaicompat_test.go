...
const measuredToolCallAnswer = `{
  ...
  "choices": [{
    ...
    "message": {
      "role": "assistant",
      "tool_calls": [{
        "function": {"name": "describe_session", "arguments": "{}"},
        "type": "function",
        "id": "5a5cf78e"
      }]
    }
...
func TestOpenAICompatible_ReadsAToolCall(t *testing.T) {
	...
	c.Assert(answer.Message.ToolCalls[0].Name, qt.Equals, "describe_session")
	...
	_, err := provider.Chat(context.Background(), aiprovider.Request{
		Messages: []aiprovider.Message{
			...
			{Role: aiprovider.RoleAssistant, ToolCalls: []aiprovider.ToolCall{{
				ID: "call-1", Name: "describe_session", Arguments: json.RawMessage(`{}`),
			}}},
			{Role: aiprovider.RoleTool, ToolCallID: "call-1",
				ToolName: "describe_session", Content: `{"artifacts":[]}`},
		},
	})

...
// TestLoadFSDirectiveSectionParsing_HappyPath pins the goose and dbmate
// directive parsers against behavior measured on the pinned community binary
// (v1.3.0), file by file and byte for byte.
//
...
//
// The files this parser refuses are in
// TestLoadFSDirectiveSectionParsing_FailurePath.
func TestLoadFSDirectiveSectionParsing_HappyPath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantBody: "CREATE TABLE a (id INTEGER PRIMARY KEY);\n\nCREATE TABLE b (id INTEGER PRIMARY KEY);\n",
		},
		{
			...
			wantBody: "CREATE TABLE pre (id INTEGER PRIMARY KEY);\n" + widgets,
		},
		{
		...
		},
		{
			...
			wantBody: "-- +goose up to date\n" + widgets,
		},
		{
			// An empty up SECTION is legitimate and is recorded, exactly as the
			// community binary records it. A dbmate file with no up directive at
			// all is refused instead; see the failure-path test.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Assert(string(loaded.Entries[0].Data), qt.Equals, tt.wantBody)
		})
	}
}

// TestLoadFSDirectiveSectionParsing_FailurePath pins the files the parsers
...
//
// The files these same parsers convert are in
// TestLoadFSDirectiveSectionParsing_HappyPath, which is what keeps a refusal
// here from being satisfiable by a parser that refuses everything.
func TestLoadFSDirectiveSectionParsing_FailurePath(t *testing.T) {
	const widgets = "CREATE TABLE widgets (id INTEGER PRIMARY KEY);\n"
	...
	tests := []struct {
		...
		// --- out-of-order directives: the community binary refuses these too ---
		{
			...
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose Down\nDROP TABLE widgets;\n-- +goose Up\n" + widgets,
			wantErr: `migration file 1_init\.sql line 1: unexpected "-- \+goose Down" directive because no up section has been opened yet`,
		},
		{
			name:    "goose refuses a second Up",
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose Up\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n-- +goose Up\nCREATE TABLE b (id INTEGER PRIMARY KEY);\n",
			wantErr: `migration file 1_init\.sql line 3: unexpected "-- \+goose Up" directive because an up section is already open`,
		},
		{
			name:    "goose refuses StatementEnd with no StatementBegin",
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose Up\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n-- +goose StatementEnd\n",
			wantErr: `migration file 1_init\.sql line 3: unexpected "-- \+goose StatementEnd" directive because no "-- \+goose StatementBegin" block is open`,
		},
		{
			name:    "goose refuses StatementBegin outside a section",
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose StatementBegin\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n-- +goose StatementEnd\n",
			wantErr: `migration file 1_init\.sql line 1: unexpected "-- \+goose StatementBegin" directive because no up section has been opened yet`,
		},
		{
			...
			file: "-- +goose Up\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n" +
				"-- +goose StatementBegin\n-- +goose Up\nSELECT 1;\n-- +goose StatementEnd\n",
			wantErr: `migration file 1_init\.sql line 4: unexpected "-- \+goose Up" directive because it appears inside a "-- \+goose StatementBegin" block`,
		},
		...
		// --- deliberate divergences: the community binary exits 0 on these ---
		{
			...
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose Up\nCREATE TABLE a (id INTEGER PRIMARY KEY);\n-- +goose down\nDROP TABLE a;\n",
			wantErr: `(?s)migration file 1_init\.sql line 3: "-- \+goose down" is not a goose directive .*Write "-- \+goose Down" instead`,
		},
		{
			name:    "goose refuses a lowercase up as a near miss",
			format:  atlasmigrateimport.FormatGoose,
			file:    "-- +goose up\n" + widgets,
			wantErr: `(?s)migration file 1_init\.sql line 1: "-- \+goose up" is not a goose directive .*Write "-- \+goose Up" instead`,
		},
		{
			...
			format:  atlasmigrateimport.FormatDBMate,
			file:    widgets,
			wantErr: `migration file 1_init\.sql carries no "-- migrate:up" directive.*`,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := atlasmigrateimport.LoadFS(source, "migrations", tt.format)

...
// private copy would drift from the pinned version constant, and the whole
// value of these runs is that they measure the SAME build.
...
func TestOracleKeepsTheIndexStorageParameterPtahRenders(t *testing.T) {
	...
	result, err := RenderInspectedForAtlasCLI(brinStorageParamDocument(schema), platform.Postgres, schema)
	c := qt.New(t)

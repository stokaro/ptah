...
//
// IDENTITY_COLUMN says whether a column is an identity and not how it is
// generated. ALL_TAB_IDENTITY_COLS.GENERATION_TYPE carries that, and it decides
// whether the column takes an explicit value: GENERATED ALWAYS refuses one with
// ORA-32795, GENERATED BY DEFAULT accepts it. The join is LEFT because most
// columns are not identities and have no row there.
//
...
func (r *Reader) readColumns(ctx context.Context) (map[string][]catalog.Column, error) {
	...
	for rows.Next() {
		...
		column.IdentityGeneration = identityGeneration(generation)
...
//
// IsPrimaryKey and IsUnique are derived fields on Column, and every reader
...
func assignDefault(column *catalog.Column, identity, virtual bool, def sql.NullString) {
	...
	case identity:
		// IsAutoIncrement and IdentityGeneration already carry this fact.
	default:
		column.ColumnDefault = value
	}
}

// identityGeneration spells ALL_TAB_IDENTITY_COLS.GENERATION_TYPE the way the
// catalog records it: "ALWAYS", "BY_DEFAULT", or "" for a column that is not an
// identity. Oracle writes the second one with a space.
//
// GENERATED BY DEFAULT ON NULL reports BY DEFAULT here, measured on 23.26, and
// it accepts an explicit value like BY DEFAULT does, so the ON NULL half does
// not change the answer.
//
// A value this function does not know is left empty rather than guessed. An
// empty generation on an identity column reads as "not known", which no caller
// takes for "rejects explicit inserts".
func identityGeneration(value sql.NullString) string {

...
// returns a whole Database whose other members would have to be faked to reach
// this one field.
//
...
// that came from the currentCatalog from one synthesized downstream.
...
func pgNamespaceServer(currentCatalog map[string][][]driver.Value) dbtest.QueryHandler {

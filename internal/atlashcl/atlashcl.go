...
func (p *parser) parseTable(block *hclsyntax.Block) error {
	...
	for _, nested := range block.Body.Blocks {
		if nested.Type == dynamicBlockType {
			if err := p.expandDynamic(nested, func(desired *hclsyntax.Block) error {
				if desired.Type == "check" && len(desired.Labels) == 0 {
					unlabeledCheckOrdinal++
				}
				return p.parseTableBlock(&table, fieldsStart, unlabeledCheckOrdinal, desired)
			}); err != nil {
				return err
			}
			continue
...
func (p *parser) parseColumn(structName string, block *hclsyntax.Block) (schemamodel.Field, error) {
	...
	desired, err := p.parseGeneratedColumn(block)
	if err != nil {
		return schemamodel.Field{}, err
	}
	...
	if desired.expression != "" && identity.generation != "" {
...
func (p *parser) parseIdentityColumn(block *hclsyntax.Block) (identityColumnSpec, error) {
	...
	desired := p.optionalString(identityBlock.Body.Attributes["generated"])
	generation := normalizeIdentityGeneration(desired)
	if generation == "" {
		if desired != "" {
			return identityColumnSpec{}, p.blockError(identityBlock, "unsupported identity generated value %q", desired)
		}

...
func TestFunctionDiff_BodyAndSecurityChange_Integration(t *testing.T) {
	...
	c.Assert(beforeFn.Body, qt.Contains, "false")
	...
	c.Assert(sqlForAssert, qt.Contains, "CREATE OR REPLACE FUNCTION ptah_test_set_group_context")
	c.Assert(sqlForAssert, qt.Contains, "set_config('app.current_group_id', group_id_param, true)")
	c.Assert(sqlForAssert, qt.Not(qt.Contains), "SECURITY DEFINER",
...
func TestFunctionDiff_VolatilityChange_Integration(t *testing.T) {
	...
	sqlText := strings.Join(statements, ";\n") + ";" //nolint:gosec // Statements are generated by Ptah's SQL planner for this live round trip.
	c.Assert(sqlText, qt.Contains, "STABLE")

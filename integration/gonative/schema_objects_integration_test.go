...
func TestSchemaObjects_RoundTripAndBodyChange_PostgreSQL_Integration(t *testing.T) {
	...
	modifiedStatements, err := planner.GenerateSchemaDiffSQLStatements(bodyDiff, "postgres")
	c.Assert(err, qt.IsNil)
	...
	c.Assert(removalDiff.MaterializedViewsRemoved.Names(), qt.DeepEquals, []string{"ptah_schema_objects_user_stats"})
	...
	removalStatements, err := planner.GenerateSchemaDiffSQLStatements(removalDiff, "postgres")
	c.Assert(err, qt.IsNil)

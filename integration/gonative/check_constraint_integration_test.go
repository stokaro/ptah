...
func TestFieldLevelCheckConstraint_RoundTrip_Integration(t *testing.T) {
	...
	c.Assert(sqlForAssert, qt.Contains, "CONSTRAINT ptah_test_files_kind_valid CHECK")
...
func TestFieldLevelCheckConstraint_Removal_Integration(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	migrationSQL := strings.Join(statements, ";\n") + ";" //nolint:gosec // Statements are generated by Ptah's SQL planner for this live round trip.
	c.Assert(migrationSQL, qt.Contains, "ptah_test_check_drop_score_check",

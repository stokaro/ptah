...
type Product struct {
	...
	//ptah:schema:field name="stock" type="INTEGER" not_null="true" default_expr="0" check="stock >= 0"
	Stock int

	// New column with an explicit `check_name=` so we can exercise the
	// "trust the name" matching during a later rename (in 020).
	//ptah:schema:field name="status" type="VARCHAR(32)" not_null="true" default="active" check="status IN ('active','draft','archived')" check_name="products_status_valid"
	Status string

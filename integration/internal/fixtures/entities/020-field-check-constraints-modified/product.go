...
type Product struct {
	...
	//ptah:schema:field name="price" type="DECIMAL(10,2)" not_null="true" check="price > 0" check_name="products_price_positive"
	Price float64

	// stock: CHECK annotation dropped. Diff should produce one
	// ConstraintsRemoved targeting `products_stock_check`.
	//ptah:schema:field name="stock" type="INTEGER" not_null="true" default_expr="0"
	Stock int
	...
	//ptah:schema:field name="status" type="VARCHAR(32)" not_null="true" default="active" check="status IN ('active','archived')" check_name="products_status_valid"
	Status string

...
//
// Measured against Oracle 23.26.2.0.0, one process, the same URL, with no Ptah
// code involved beyond the blank import:
//
//	go-ora v3.0.1   first handle ok, second handle -> EOF
//	go-ora v2.9.0   first handle ok, second handle ok, both query
//
// The same shape on Oracle 21.3.0.0.0 answers ok on both drivers, which is why
// a test that ran only there would agree with a build that cannot read a 23
// catalog (stokaro/ptah#1875).
func TestOracleSecondConnectionE2E(t *testing.T) {
	...
	// And the first still works after the second has been used, so the pair is
	// two live sessions rather than one handed back and forth.
	var answer int

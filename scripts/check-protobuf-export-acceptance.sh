...
# A baseline written in the version 1 layout must be migrated, not refused: the
# run has to succeed, keep every pinned number, and rewrite the file in the
# version 2 shape. The version 1 file is produced from the current export rather
# than checked in, so the case never drifts away from the schema next to it.
migration_root="$workspace/cases/version-one-baseline"
mkdir -p "$migration_root"
cp -R "$baseline" "$migration_root/proto"
python3 "$repo_root/scripts/downgrade-protobuf-export-to-v1.py" \
	"$migration_root/proto/$proto_relative_path"
if ! grep -q '^// ptah:protobuf-export-version=1$' "$migration_root/proto/$proto_relative_path"; then
	printf 'version 1 fixture was not produced\n' >&2
	exit 1
fi
cp -R "$migration_root/proto" "$migration_root/before"
export_schema "$repo_root/stubs" "$migration_root/proto"
check_proto_module "$migration_root/proto"
# Same schema, so the migrated file must equal what a fresh export writes: every
# number survived the rewrite.
cmp "$baseline/$proto_relative_path" "$migration_root/proto/$proto_relative_path"
(
	cd "$migration_root/proto"
	buf breaking --against "$migration_root/before"
)

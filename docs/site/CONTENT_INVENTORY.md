...
| `reference/command-flags.md` (generated) | CI author, script author, anyone reading a `PTAH_*` variable in a pipeline | Which flags does this command take, what does each default to, and which environment variable does it read? | reference | the cobra trees themselves: `internal/cmd/cmdref` walks both binaries through `internal/agentsurface` and renders the whole page, gated by `scripts/check-command-reference.sh` | `reference/native-commands` and `reference/atlas-commands` (what each verb is for; their command indexes come from the same generator), `reference/configuration` (precedence between a flag, a variable and a project file) | created (the flag reference the two command pages pointed at `--help` for); keep — it is generated in full, so a flag added to any command cannot leave the page behind |

...
- A changeset's rollback is left out, since an Atlas migration is up-only, and
  the warning about dropped rollbacks names the changeset and its file. This
  holds for `--rollback` lines, for a `/* liquibase rollback` block and for the
  `rollback` of an XML, YAML or JSON changeset. A rollback of `empty` or
  `not required` runs nothing, and is not named. The block ends at the first
  line that ends in `*/`, as Liquibase ends it, which is not always where a SQL
  comment ends.

...
`atlas <command> ...` invocations on this page run the separate `ptah-compat`
drop-in binary installed under the name `atlas`; see the
...
and `dbmate` values while evaluating `atlas.hcl`, and `atlas migrate apply`
...
Apply and `atlas migrate import` share one format-loading implementation,
...
Use Atlas project flags on commands under `atlas schema ...` and
`atlas migrate ...`:

```bash
atlas schema inspect --config project.hcl --env local
atlas migrate apply -c project.hcl --env local
atlas migrate hash --env local --var dir=migrations
```

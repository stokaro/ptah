...
audience:
  - "atlas-migrator"
  - "evaluator"
readerQuestion: "What does the current Atlas conformance evidence establish?"
goal: "Interpret the current Atlas conformance evidence and its limits."
sourceOfTruth:
  - "internal/cli/atlas"
...
Those reports are the live source for the observation counts, so this page
names them rather than restating a number that changes without it.
`gaps-live.md` carries the live database round-trip corpus and `gaps-diff.md`
the Atlas CE differential corpus.
...
dialect feature, and output mode. Use the
[Feature matrix](../feature-matrix/) for product and coverage gaps that are
outside the current measured corpus; each row that is partial names the issue
that owns it.

A green documentation build proves that the documentation site builds and that
its internal links resolve. It is not parity evidence.
...
| Gate type | Meaning |
| --- | --- |
| <GlossaryTerm term="regression-budget">Regression budget</GlossaryTerm> | No new gaps beyond the accepted budget for that <GlossaryTerm term="contour" />. Should stay green. |
| <GlossaryTerm term="full-conformance">Full conformance</GlossaryTerm> | Every checked case in that <GlossaryTerm term="contour" /> passes. May stay red while the measured corpus still has non-OK results. |
...
```bash
go test ./internal/cli/atlas -run '^TestCompatMigrateNew' -count=1
...
| [Pre-approved direct schema plans](#pre-approved-direct-schema-plans) | Same engine as `schema apply` | `schema plan` aborts |
...
[Lint rules](../../reference/lint-rules/). Four verbs sit outside the pin
entirely and are triaged rather than measured here; see
[Verbs beyond the CE pin](#verbs-beyond-the-ce-pin).
...
**Native Ptah.** Ptah plans and applies direct schema changes through the same engine that powers `schema apply`
...
absent from the pinned conformance Atlas CE binary — each resolves to
`unknown command` rather than to a community-version abort stub — so they are
outside the CLI-surface parity target. Being outside that target is not a
reason to leave a capability unbuilt: all four are Ptah verbs, reachable
through the compatibility spelling and omitted only under
`PTAH_ATLAS_STRICT_COMPAT=1`, which is the profile that mirrors the pin.

| Verb | Where it is answered |
| --- | --- |
| `migrate ls` | `ptah-compat migrate ls`, forwarding to native `ptah migrations ls`. Not `ptah migrations status`: status needs `--db-url` and reports applied and pending state, while listing a directory is a local question with no database in it. |
| `migrate show` | `ptah-compat migrate show`, forwarding to native `ptah migrations show`, which prints a stored migration's SQL. |
| `schema validate` | `ptah-compat schema validate` and native `ptah schema validate` report every structural problem in a desired state without a database. |
| `schema stats` | `ptah-compat schema stats inspect` and native `ptah schema stats`, which count the objects in a live schema and emit them as OpenMetrics. |

Revisit the classification when the conformance Atlas pin advances.
[`stokaro/ptah#1618`](https://github.com/stokaro/ptah/issues/1618) tracks it.

...
Atlas's external migration-tool `--dir-format` values are read directly by `hash`, `validate`, `lint`, `status`, and `set`, under both that spelling and `?format=` on `--dir`. `migrate new` and `migrate diff` also write the selected layout; diff emits each layout's forward and rollback artifact shape and refreshes `atlas.sum` over its covered set.

That closes the layout-selection and artifact-shape gap, not every reverse-planning gap. Foreign-layout rollback does not yet carry native MySQL/MariaDB foreign-key backing-index cleanup or PostgreSQL concurrent-index reversal refinements. `atlas.sum` validation proves the emitted bytes are internally consistent; it does not prove those rollback semantics.

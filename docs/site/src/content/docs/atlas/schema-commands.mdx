...
import schemaDiffWeb from '../../../assets/schema-diff-web.png';

export const schemaDiffWebSample = `${import.meta.env.BASE_URL}samples/reports/schema-diff-web.html`;
...
For a picture instead of a file, `--web` writes the schema as a local HTML
document and opens it. See [Look at a schema as a
diagram](#look-at-a-schema-as-a-diagram).
...
with Atlas's `--dev-url cannot be empty` message, and with Atlas's link to its
dev-database page when the source is SQL or a migration directory. A
`--dev-url` made of spaces is a value, and fails as Atlas fails it:
...
a `main.sql` `atlas:import` entry point for SQL, which reads back as the whole
export), `"schema"` (one file per schema), and `"type"` (one file per object
type), plus an optional file-extension argument:
...
Use `--tx-mode=file`, the default, to execute the generated plan in one
transaction, or `--tx-mode=none` to execute statements without transaction
wrapping. Those are the two values Atlas accepts here, spelled exactly; any
other, `all` and an empty value included, is refused with Atlas's
`unknown tx-mode "<value>"`. A saved plan applied with `--plan` also takes
`all`, where it refuses a `-- atlas:txmode` directive. With `--edit`, the planned SQL opens in `$VISUAL` or `$EDITOR`
...
`--dev-url` must match the target database dialect. It is required when
`--to` is SQL a dev database has to run first — a SQL file, a directory of SQL
files, a migration directory, or an `env://` reference to one of those — and
without one the apply is refused before the target is contacted, with Atlas's
`--dev-url cannot be empty. See: https://atlasgo.io/atlas-schema/sql#dev-database`.
A database
`--to`, an HCL file or a directory of HCL files needs none, as on Atlas, and
neither does a YAML schema file: each is already a schema definition.
`PTAH_ATLAS_APPLY_WITHOUT_DEV_URL=1` plans a SQL file or directory with no dev
database, which Atlas refuses; native `ptah schema apply` has no such
requirement.
...
It is a duration and defaults to `10s`, as on Atlas. Zero or a negative value
does not wait: the lock is tried once, and a lock another apply holds fails
the run at once, which is how Atlas reads the same values. A value that is not
a duration, an empty one included, is refused by the flag parser with Atlas's
`invalid argument "…" for "--lock-timeout" flag` sentence. An elapsed timeout
fails the apply before the locked target reinspection; a strict target-policy
refusal happens before the timeout is consulted. PostgreSQL and YugabyteDB (`pg_advisory_lock`),
MySQL and MariaDB (`GET_LOCK`), and SQL Server (`sp_getapplock`) use real
database locks.

Every other dialect has no advisory-lock semantics: the apply proceeds without
a lock, and an explicitly passed `--lock-timeout` prints a note on stderr. The
native `ptah schema apply` refuses a typed `--lock-timeout` on those dialects
instead — see [Apply a desired schema](../../direct/apply/).
...
own base64 hashes (those have no local recipe — in either direction). Without
`--save`/`--output`/`--dry-run`, the plan document prints to stdout, and
`--auto-approve` is accepted for CLI compatibility.

An `--output` path ending in `.json` writes the native JSON plan
(`format_version` 1) instead, which additionally records per-statement safety
severity, the dialect, exclude patterns, and the schemas the plan reads beyond
a `--from` URL pinned to one schema. The `.plan.hcl` shape has no field for
either of the last two, so a plan that needs them is refused in that shape.
...
`--dev-url` first, then by `--from` and `--to` database URLs. With `--env`,
...
A side that is a schema file, a schema directory or a migration directory needs
`--dev-url`, as on Atlas, and without one the diff is refused before any
database is contacted; two database URLs need none. The message is Atlas's
`--dev-url cannot be empty`, with Atlas's link to its dev-database page when the
first side that needs one is SQL or a migration directory. Set
`PTAH_ATLAS_DIFF_WITHOUT_DEV_URL=1` to compare a schema file with a database
without a dev database, as native `ptah schema diff` does; see
[Extension environment
variables](../../reference/extension-variables/#ptah_atlas_diff_without_dev_url).
...
before any database is contacted. Non-Atlas-CE flags such as `--tx-mode` are
rejected as unknown.
...
## Look at a schema as a diagram

`-w`/`--web` on `schema inspect` and `schema diff` writes the schema as a
self-contained HTML document and opens it with whatever the desktop opens HTML
with. On a diff the document carries the comparison: every table is marked
added, changed or removed, and one both sides agree on is left plain.

```bash
ptah-compat schema diff \
  --from file://erd-from.sql \
  --to file://erd-to.sql \
...
<ProductPreview
  id="schema-diff-web"
  src={schemaDiffWeb}
  alt="A Ptah schema diff document: a dependency diagram whose four table boxes are colored green for the added table, amber for the changed one, red for the removed one and plain for the untouched one, above a legend naming the three marks."
  caption="A real `schema diff --web` document for a comparison that adds one table, changes one, and removes one."
  notice="The color is on the boxes of the dependency diagram, not in a separate list, so the change and what depends on it are read in one picture. The legend names only the marks this comparison produced."
  fullSizeHref={schemaDiffWebSample}
  downloadHref={schemaDiffWebSample}
  sourceHref={schemaDiffWebSample}
  reproduce={`ptah-compat schema diff --from file://docs/site/fixtures/product-output/erd-from.sql --to file://docs/site/fixtures/product-output/erd-to.sql --dev-url "sqlite://dev?mode=memory" --web`}
/>

A removed table is in neither `--to` nor a document rendered from it, so the
diagram is drawn from the end state plus the tables that leave it. That is why
`legacy_notes` appears at all.

The migration SQL still goes to standard output, unchanged. The document's path
goes to standard error, so `--web` composes with a run whose output is being
read:
...
Atlas's published CLI reference describes `-w, --web` as opening the schema ERD
in the browser, which a reader may reasonably take to mean a hosted page. Ptah
writes a local file and opens that. A schema is the shape of somebody's data, so
the flag that shows it must not be the flag that publishes it. The document
fetches no stylesheet, font, script or image, and a database URL is reduced to
its scheme and host before it is written into the file, so a password does not
travel with a document somebody attaches to a review.

The pinned Atlas community binary v1.3.0 does not register the flag: `atlas
schema inspect --url ... --web` answers `unknown flag: --web`. So there is no
community behavior this diverges from, and `PTAH_ATLAS_STRICT_COMPAT=1` answers
`unknown flag` here too. `migrate lint --web` stays unregistered.

### When no browser can open

A run that cannot open a browser is not a failed run: the document is the
deliverable and it is on disk before anything is opened. Under `CI`, or with no
display, the path is printed with the reason nothing opened, and the command
exits `0`:
...
`PTAH_SKIP_BROWSER_OPEN=1` asks for the same on a machine that could have opened
one. It is a variable rather than a flag because the conformance `cli-surface`
tier asserts exact flag parity with the pinned binary. Like every boolean
`PTAH_*` variable it is strict: a value that is not a boolean is refused before
the command does any work.

The native spelling of the same artifact is [`ptah schema export --to html
--open`](../../schema/document/#open-it-straight-away), which uses the same
renderer and the same opener.

...
| Docker dev databases (`docker://` `--dev-url`) | 🟡 | ✅ | ✅ | migrate diff, lint, validate, schema inspect, schema diff, apply, both test verbs, checkpoint and down start a container and remove it. `schema plan` dialect-checks the value and never provisions it. |

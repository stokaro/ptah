...
| Atlas CE strict oracle profile | ✅ | ➖ | ➖ | Strict mode builds the CE tree and refuses unsafe sources, forwarded Ptah environment aliases, lint policy, migration extensions, and writer-only live objects before work. Cleanup validation follows the writer's schema scope and distinguishes implicit from standalone sequences. Default mode retains the full surface and selector coverage. |
...
| Spanner PostgreSQL interface (spanner) | 🟡 | ❌ | ✅ | Enums, sequences, matviews, functions and triggers render as named skip comments; foreign keys render. SERIAL hard-errors, no dedicated driver (PostgreSQL pgx path), no live container or test. |

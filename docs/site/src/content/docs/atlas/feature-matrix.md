...
| Roles, grants, and row-level security | ✅ | ❌ | ✅ | Roles and grants on every engine that has them except Oracle, where Ptah renders neither yet. Row-level security on the PostgreSQL family, SQL Server and ClickHouse; the rest have no such object. |

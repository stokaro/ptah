...
| External `--dir-format` outside `migrate import` | 🟡 | ✅ | ✅ | Five external layouts read and write. Goose, dbmate and Liquibase carry a no-transaction requirement in their own syntax; golang-migrate and Flyway do not yet (stokaro/ptah#1630). |
...
| Query builder for parameterized SQL | 🟡 | ➖ | ➖ | Joins, DISTINCT, GROUP BY, HAVING, RETURNING and LIKE work across every dialect; no subqueries, CTEs or upsert. ClickHouse UPDATE/DELETE are mutations and refuse. Tracked by stokaro/ptah#941. |

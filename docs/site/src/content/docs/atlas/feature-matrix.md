...
| Functions | 🟡 | ❌ | ✅ | `schema render` and `schema apply` emit CREATE FUNCTION for PostgreSQL, CockroachDB, YugabyteDB, MySQL and MariaDB. The MySQL family reads routines back and plans changes, so a declared function converges. A `language` other than `sql` is named as skipped there. Spanner names the function as skipped in both. |
| MySQL and MariaDB | 🟡 | ✅ | ✅ | Stored functions render, read back and plan; volatility, security mode and routine types survive the round trip. Matviews are an explicit error on both `render` and `apply`; extensions and MariaDB SEQUENCE objects get a not-supported comment on both. Domains, roles/grants and RLS still drop silently. |

...
| Atlas-format checkpoint output | ❌ | ❌ | ✅ | migrate checkpoint `--dir-format`=atlas is a recorded waiver; Ptah writes only the ptah two-file checkpoint convention. Reading Atlas `-- atlas:checkpoint` directories works: fresh databases bootstrap from the latest checkpoint, pre-checkpoint databases skip it. |

...
Across the 175 capabilities below:
...
| An Atlas column is ❔ — not established by this page's evidence | 6 |
...
| Exclude selector that matches nothing is diagnosed | ✅ | ❌ | ❔ | A selector naming no object warns on inspect and diff and exits 1 on apply; a PTAH_ATLAS opt-in restores the permissive behavior. Deliberate: the community binary is silent about it. |

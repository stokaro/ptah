...
| Roles, grants, and row-level security | 🟡 | ❌ | ✅ | PostgreSQL emits all three; YugabyteDB refuses RLS; CockroachDB and Spanner refuse both, as a named skip comment on `render` and `apply` alike. Atlas prices this as Pro; CE no-ops `role` silently. |

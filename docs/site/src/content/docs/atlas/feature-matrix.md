...
| Atlas-shaped migrate test / schema test verbs | 🟡 | ❌ | ✅ | Reports and seed directories are exposed. schema test accepts `-s/--schema`, Go, SQL/HCL or database sources; `--var` reaches HCL files. `env://` sources are refused by name. |
...
| env:// desired-state references | 🟡 | ✅ | ✅ | Resolves on `--to`/`--from`/`--url`, over six attributes. Every other flag refuses the scheme by name rather than using it literally: `--schema-file`, the test verbs' sources, `--exclude`/`--include`. |

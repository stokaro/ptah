...
| Generate migrations from a schema diff | 🟡 | ✅ | ✅ | New versions are the UTC YYYYMMDDHHMMSS stamp on every verb that writes one, stepping only past a version already taken, as CE does. Docker dev databases remain a follow-up gap. |

...
| External `--dir-format` outside `migrate import` | 🟡 | ✅ | ✅ | Query and flag selection cover every Atlas OSS layout, including new and diff output. Rollback still omits MySQL/MariaDB FK-index cleanup and PostgreSQL concurrent reversal. |

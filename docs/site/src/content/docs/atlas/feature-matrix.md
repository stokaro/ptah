...
| data "sql", "external", "runtimevar", "template_dir", "remote_dir" | ❌ | ✅ | ✅ | Ptah resolves no other data source than `hcl_schema` and `external_schema`. Declaring one is fine — a project file that never reads it, or reads it only from an environment the run did not select, parses as on CE. Reading one fails, naming the construct and its line. Tracked in [#1511](https://github.com/stokaro/ptah/issues/1511). |

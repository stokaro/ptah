...
| Apply advisory lock, `--lock-timeout`, `--lock-name`, `--skip-lock` | ✅ | 🟡 | ✅ | Real locks on PostgreSQL, YugabyteDB, MySQL, MariaDB, SQL Server. ptah-compat notes and runs others unlocked; native ptah refuses a typed `--lock-timeout`. `--lock-name`/`--skip-lock` are Pro surface. |
...
| Exclude selector that matches nothing is diagnosed | ✅ | ❌ | ❌ | A selector naming no object warns on inspect and diff and exits 1 on apply, and only a filter that asked it may call it empty. `PTAH_ATLAS_ALLOW_UNMATCHED_EXCLUDE` restores permissive behavior. |
...
| Go-template `--format` output | ✅ | ✅ | ✅ | schema apply registers the shared helper set; schema inspect registers its own (base64url, hcl, json, mermaid, sql, split, write). schema diff registers `sql` by default and the rest behind an opt-in. |
...
| External `--dir-format` outside `migrate import` | ✅ | ✅ | ✅ | Five external layouts read and write on `migrate new` and `migrate diff`, each with a no-transaction requirement in its own format. `edit`, `rebase`, `rm`, `test` and `checkpoint` refuse them by name. |
...
| Migration lock, lock timeout, `--lock-name`, `--skip-lock` | ✅ | 🟡 | ✅ | Compat `--lock-timeout` bounds directory and dev-db locks; native `migrations up`/`down`/`baseline` refuse `--migration-lock-timeout` where the dialect cannot lock. `--lock-name` is Pro. |

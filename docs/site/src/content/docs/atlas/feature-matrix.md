...
| Functions | 🟡 | ❌ | ✅ | `schema render` and `schema apply` emit CREATE FUNCTION for PostgreSQL, CockroachDB, YugabyteDB, MySQL and MariaDB; the MySQL family reads them back and plans changes too. Spanner names the function as skipped. |
| MySQL and MariaDB | 🟡 | ✅ | ✅ | Stored functions render, read back and plan. Matviews are an explicit error on both `render` and `apply`; extensions and MariaDB SEQUENCE objects get a not-supported comment on both. Domains, roles/grants and RLS still drop silently. |

...
| Directory integrity file: hash, validate, and the apply/status/set gate | ✅ | ✅ | ✅ | hash writes `ptah.sum` or `atlas.sum`; validate checks it. Compat apply, status and set refuse a stale or missing `atlas.sum`; apply also via `?format=`. Native up gates hashed dirs only. |
...
| Atlas-shaped migrate test / schema test verbs | 🟡 | ❌ | ✅ | schema test -u takes a Go-annotation directory, a .sql or .hcl schema file, or a database URL; env:// and atlas:// are refused. Neither verb exposes `--report` or `--seed-dir`. |
...
| Go annotations to HCL export with cleanup | ✅ | ➖ | ➖ | Writes HCL from Go annotations; cleanup requires zero diagnostics. Function, view, materialized-view, and trigger bodies are emitted as opaque HCL text, reported, and block cleanup. |

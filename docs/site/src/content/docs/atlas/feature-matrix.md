...
| Directory integrity file: hash, validate, and apply-time gate | ✅ | ✅ | ✅ | hash writes `ptah.sum` or `atlas.sum`; validate checks it. Compat apply refuses a stale or missing `atlas.sum` like Atlas; native up gates hashed dirs only, unless `--verify-sum`. |

...
| Report of an ignored `--dir` URL query key | ✅ | ❌ | ❌ | Only `?format=` selects a layout. Every other key is ignored on both binaries; ptah-compat names it on stderr and still exits 0, and `PTAH_STRICT_DIR_QUERY=1` refuses it instead. |

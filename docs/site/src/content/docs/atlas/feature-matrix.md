...
| Migration linting | ✅ | 🟡 | ✅ | CE registers `migrate lint` with a basic Open rule set; its features page marks the lint CLI Pro. Compat requires `--dev-url` as CE does; `PTAH_ATLAS_LINT_WITHOUT_DEV_URL` lifts that. |

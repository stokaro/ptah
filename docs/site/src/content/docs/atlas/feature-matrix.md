...
| An include selector matching neither diff side fails closed | ✅ | ❌ | ❔ | The full compat and native diff surfaces retain `--include`, including real dotted top-level names, but a selector matching neither side exits nonzero with no false synced report. |

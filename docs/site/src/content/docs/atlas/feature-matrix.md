...
| SQL Server and Azure SQL (sqlserver, mssql, tsql) | 🟡 | ❌ | ✅ | Every accepted spelling renders the same DDL, and SQL Server is in `schema render`'s dialect list and `--dialect` help. Sequences render, read back and plan; RLS, roles/grants and matviews do not. |

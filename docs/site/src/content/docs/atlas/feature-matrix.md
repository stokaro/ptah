...
| External `--dir-format` outside `migrate import` | 🟡 | ✅ | ✅ | hash, validate, lint, status and set read every Atlas source layout, under both `?format=` and `--dir-format`. `migrate new` and `migrate diff` take only atlas and refuse every `--dir` query (#1086). |

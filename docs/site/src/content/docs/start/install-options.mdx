...
```powershell
irm https://ptah.run/install.ps1 -OutFile install.ps1
...
```bash
go install ptah.run/cmd/ptah@vX.Y.Z
```

Install the compatibility binary and language server separately:

```bash
go install ptah.run/cmd/ptah-compat@vX.Y.Z
go install ptah.run/cmd/ptah-ls@vX.Y.Z
```

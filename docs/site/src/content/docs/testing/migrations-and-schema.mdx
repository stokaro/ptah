...
| Source | Legacy selector | Source-specific limitation |
| --- | --- | --- |
| SQL file | `--root-dir ./schema.sql` | Uses Ptah's DDL parser subset; the historical flag name does not make the file a directory. |
| YAML file | `--root-dir ./schema.yaml` | Uses Ptah YAML schema objects; the suffix selects YAML. |
| HCL file | `--root-dir ./schema.hcl` | Uses the Atlas-compatible HCL subset plus Ptah extensions. |
| DBML file | `--root-dir ./schema.dbml` | DBML cannot express every Ptah object. |
| Go annotations | `--root-dir ./models` | Uses the native Go annotation model and requires a Go source tree. |
| Live database | `--root-dir sqlite://source.db` | The database reader measures the live catalog; the destination must be throwaway and a non-SQLite source needs an explicit matching `--db-url`. |
...
`--root-dir` also accepts a file or a database URL, which is what it did before
the other two selectors existed. Such a run works and reports on standard error
which selector names that source exactly.

...
The command also prints a warning on stderr that names `PG101`. The rule can
read a dev database, given with `--dev-url`, to tell whether the table is a
TimescaleDB hypertable, and this run gave none. For an ordinary table the

...
something, and convert each result separately. `--dialect` does not choose
among `dbms` values: the name Liquibase gives some databases depends on how it
connected, so Ptah cannot tell which changesets ran on yours.
...
runs once. The value `false` is the default and imports.

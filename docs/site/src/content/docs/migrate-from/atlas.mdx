...
The remedy names the `atlas` binary because the whole message is Atlas's, byte
for byte. A CI job that greps this text keeps working, which is the point of
matching it rather than improving it.
...
| --- | --- |
| A hosted registry holding migration directories and schemas | An OCI registry you already run. `ptah oci` carries `login`, `inspect`, `fetch`, `copy`, `tag`, `tags`, `resolve`, `referrers`, `reindex`, `verify` and `capabilities`, including the air-gapped path. |

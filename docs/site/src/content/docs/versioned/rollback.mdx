...
A migration directory and a database with applied migrations. The examples use
a local SQLite file with two applied migrations, `1785255952_init` and
`1785255953_add_posts`.

If you do not have that state, build it here. The two migrations are written by
hand so the versions match the ones this page rolls back:
...
cd ptah-rollback
mkdir migrations
cat > migrations/1785255952_init.up.sql <<'SQL'
CREATE TABLE users (
    id    INTEGER PRIMARY KEY,
    email TEXT NOT NULL
);
SQL
cat > migrations/1785255952_init.down.sql <<'SQL'
DROP TABLE users;
SQL
cat > migrations/1785255953_add_posts.up.sql <<'SQL'
CREATE TABLE posts (
    id    INTEGER PRIMARY KEY,
    title TEXT NOT NULL
);
SQL
cat > migrations/1785255953_add_posts.down.sql <<'SQL'
DROP TABLE posts;
SQL
```
...
```powershell
New-Item -ItemType Directory ptah-rollback | Out-Null
Set-Location ptah-rollback
...
'@ | Set-Content migrations/1785255952_init.up.sql
'DROP TABLE users;' | Set-Content migrations/1785255952_init.down.sql
@'
CREATE TABLE posts (
    id    INTEGER PRIMARY KEY,
    title TEXT NOT NULL
);
'@ | Set-Content migrations/1785255953_add_posts.up.sql
'DROP TABLE posts;' | Set-Content migrations/1785255953_add_posts.down.sql
```

</TabItem>
</Tabs>

Seal the directory and apply both, so there is something to roll back:
...
Database is now at version: 1785255953
```
...
Check where the database is first:
...
An interactive run states the consequences and asks for confirmation before it
does anything:
...
Type `YES` to proceed. Pass `--confirm` where there is no terminal to answer,
which is what the run below does:
...
The version is pending again, not gone, so the same `up` puts it back:
...
- `ptah migrations set --version <previous>` — move the revision boundary when
  you finished the rollback by hand. Use `--version 0` where the rolled-back
  migration is the oldest one in the directory and no previous version exists.

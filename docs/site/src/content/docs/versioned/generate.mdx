...
Start in an empty directory. The database does not need to exist yet:
...
```console
ptah migrations plan --schema-file ./schema.sql --db-url "sqlite://app.db" --report json
```
...
```console
ptah migrations generate --schema-file ./schema.sql --db-url "sqlite://app.db" --migrations-dir ./migrations --name init
```
...
statement, and reason beside the generated migration pair. It opens with how
many statements fall in each severity, which is the question a reviewer opens a
safety report to answer, and it is self-contained: opening it fetches nothing
and it follows the reader's light or dark preference.
...
<ProductPreview
  ...
  notice="The severity counts sit above the table, and the affected statement and reason are visible beside each verdict, so a reviewer does not need to infer risk from generated SQL alone."
...
The replay log comes before the generated files:

...
| Declarative schema apply | Open | [Comparison](../comparison/), [Atlas-compatible CLI](../../workflows/atlas-cli/) | Partial. `ptah atlas schema apply` reads a live database, diffs it against local `file://` `.hcl`, `.yaml`, `.yml`, or `.sql` desired schema files, prints planned SQL, supports `--dry-run`, and applies after interactive confirmation or explicit `--auto-approve`. Database desired-state URLs, migration directories, `env://`, include/exclude filters, Atlas dev-database simulation, custom formatting, lock flags, and full transaction-mode semantics remain incomplete. | Partially measured with local schema files and live SQLite apply/no-op/dry-run coverage. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510). |

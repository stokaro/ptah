...
| Schema inspection | Open | [Commands](../commands/), [Capabilities](../capabilities/), [Comparison](../comparison/) | Partial. `ptah db read` remains the native Ptah schema-read path. `ptah atlas schema inspect`, `ptah-compat schema inspect`, and renamed `atlas schema inspect` now emit Atlas-shaped output without Ptah status banners: HCL by default, SQL with `--format sql` or `--format '{{ sql . }}'`, JSON with `--format json` or `--format '{{ json . }}'`, and custom templates using the supported inspect helpers. Split/write templates, include/exclude filters, file-backed inspection, and dev-database inference remain tracked gaps. | Partially measured by live SQLite HCL/SQL/JSON/custom-template/compat probes and CLI flag probes. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510). |
| Declarative schema apply | Open | [Comparison](../comparison/), [Atlas-compatible CLI](../../workflows/atlas-cli/) | Partial. `ptah atlas schema apply` reads a live database, diffs it against local `file://` `.hcl`, `.yaml`, `.yml`, or `.sql` desired schema files, prints planned SQL, supports `--dry-run`, applies after interactive confirmation or explicit `--auto-approve`, and supports Atlas transaction modes `file`, `all`, and `none` for the generated plan. Database desired-state URLs, migration directories, `env://`, include/exclude filters, Atlas dev-database simulation, custom formatting, and lock flags remain incomplete. | Partially measured with local schema files and live SQLite apply/no-op/dry-run/transaction-mode coverage. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510). |
| Declarative schema diff | Open | [Comparison](../comparison/), [Atlas-compatible CLI](../../workflows/atlas-cli/) | Partial. `ptah schema compare` covers Ptah's native Go/live-DB comparison path. `ptah atlas schema diff` supports local `file://` schema-file diffs for `.hcl`, `.yaml`, `.yml`, and `.sql` sources plus Atlas-style SQL/custom output formatting with `--format`, `sql`, and `.MarshalSQL`. Database URLs, migration directories, `env://`, dev-database simulation, export, web output, and include/exclude remain incomplete. | Partially measured with local schema-file default, custom-template, no-op-template, and invalid-template probes. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510). |
| Desired-state sources | Open sources include SQL, HCL, and external schema integrations; some data sources are Pro | [Schema files](../../workflows/schema-files/), [Atlas HCL example](../../examples/atlas-hcl/) | Partial. Ptah supports YAML, Go annotations, supported Atlas HCL schema files, SQL schema files, live DB introspection, and SQL parsing/rendering paths. Atlas external schemas and registry-backed sources are not full parity today. | Partially measured for imported Atlas fixtures. | [`stokaro/ptah#511`](https://github.com/stokaro/ptah/issues/511). |
...
| Migration apply | Open | [Migrations](../../workflows/migrations/), [Atlas-compatible CLI](../../workflows/atlas-cli/), [Configuration](../configuration/) | Partial. `ptah migrations up` remains the native Ptah path. `ptah atlas migrate apply` and `ptah-compat migrate apply` execute Atlas-format migration directories with Atlas revision-table metadata by default and support positional `amount`, `--to-version`, `--baseline`, `--allow-dirty`, `--tx-mode`, `--exec-order`, `--revisions-schema`, `--lock-timeout`, `--lock-name`, `--dry-run`, and Go-template `--format` output over a Ptah apply result that mirrors Atlas's public apply-template fields. `--lock-name` changes the session-level advisory lock name for databases that support migration locks. | Measured for selected migration-directory and live SQLite amount, target-version, baseline, dry-run baseline, lock-name acceptance, JSON format, custom template, dry-run format, no-op format, invalid-template preflight, redacted URL, and failed-apply format cases. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510). |

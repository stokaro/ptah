...
| --- | --- | --- | --- |
| Atlas-compatible command runtime placeholders | Product behavior | These registered paths currently report that runtime behavior is not implemented yet: `ptah atlas schema apply` and `ptah atlas migrate diff`. `ptah atlas version`, `ptah atlas license`, `ptah atlas schema fmt`, and `ptah atlas migrate import` now execute Ptah-owned behavior. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510) |

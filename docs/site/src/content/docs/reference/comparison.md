...
| --- | --- | --- | --- |
| Atlas-compatible command runtime placeholders | Product behavior | These registered paths currently report that runtime behavior is not implemented yet: `ptah atlas schema apply`, `ptah atlas migrate diff`, and `ptah atlas migrate import`. `ptah atlas version`, `ptah atlas license`, and `ptah atlas schema fmt` now execute Ptah-owned behavior. | [`stokaro/ptah#510`](https://github.com/stokaro/ptah/issues/510) |

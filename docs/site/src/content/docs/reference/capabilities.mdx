...
audience:
  - "all-users"
readerQuestion: "Which capability keys does Ptah use for each dialect?"
goal: "Look up Ptah capability keys by dialect."
sourceOfTruth:
...
Ptah uses capability gates to represent dialect-specific and cross-cutting
behavior. What a capability is and how gating works is explained in
...
A cell in the matrices below is one glyph, and two different claims can wear
the same one. [The glossary](../glossary/) defines each word, from
`docs/site/src/glossary.ts` rather than from this page, so the meanings cannot
drift from the ones used elsewhere.

- A value can be measured or carried. The distinction is pinned by
  `core/platform/capability/capability_measured_lines_test.go`, which requires
  every registered key on a recorded line to be one or the other, with a
  reason attached to the second.
- A release line is certified, legacy-tested or best-effort. The level says who
  re-measures the preset, not how well the engine works.

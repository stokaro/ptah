...
- **Integrity, and its limit** — `--verify-sum` on `migrations push`, `up`, `down` and `status` requires the directory to carry a sum and to match it. A sum checks a directory against the sum stored beside it, and for an artifact that sum travels *inside* the artifact — so over a movable tag it proves the pulled files are internally consistent, not that they are the reviewed ones. Anyone who can push to the repository can rewrite the migrations, re-hash them and repoint the tag, and the check still passes. Pin a digest for authenticity; Ptah verifies no signatures.

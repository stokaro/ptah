...
- **Integrity, and its limit** — `--verify-sum` requires the directory to carry a sum and to match it. On `migrations push` the directory is the *local* one being published, checked before the upload. On `migrations up`, `down` and `status` it is the *pulled* one, and there the limit bites: a sum checks a directory against the sum stored beside it, and for an artifact that sum travels *inside* the artifact — so over a movable tag it proves the pulled files are internally consistent, not that they are the reviewed ones. Anyone who can push to the repository can rewrite the migrations, re-hash them and repoint the tag, and the check still passes. Those three verbs therefore print the resolved digest and the `@sha256:` reference that pins it. Pinning it fixes which bytes a later pull gets, which is reproducibility rather than proof of a publisher: whoever repointed the tag chose the digest you are about to pin. Ptah verifies no signatures, so publisher authenticity comes from registry access controls, a digest allowlist promoted through your deployment system, or a registry-native signature policy.

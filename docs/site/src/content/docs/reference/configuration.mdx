...
audience:
  - "all-users"
readerQuestion: "Which configuration sources does Ptah read, and in what precedence?"
goal: "Determine Ptah configuration source precedence."
sourceOfTruth:
  - "cmd"
  - "core"
  - "migration"
...
`PTAH_ATLAS_ALLOW_UNMATCHED_EXCLUDE`, `PTAH_DEV_SERVER_DISPOSABLE`,
...
`PTAH_DEV_SERVER_DISPOSABLE` declares the `--dev-url` server the run's own, so
replay runs a `DO` block, a routine or a role on it; the pinned community binary
runs those on any dev database. See

...
| `ptah atlas migrate checkpoint [name]` | Forwards to `ptah migrations checkpoint`, replaying the migration directory on the `--dev-url` dev database and writing a cumulative-schema checkpoint pair with `ptah.sum`/`atlas.sum` refreshed. `--dir` maps to the native migrations directory and the optional positional name to the checkpoint description. Atlas keeps `migrate checkpoint` in its Pro build, so this is a free Ptah capability rather than an Atlas CE stub. |

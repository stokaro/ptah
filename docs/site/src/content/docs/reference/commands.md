...
| `ptah atlas schema inspect` | Inspects a live database and writes Atlas-compatible schema output without Ptah status banners. The default output is HCL; SQL output is supported with `--format sql` or `--format '{{ sql . }}'`; JSON and custom templates are supported through `--format json`, `{{ json . }}`, `{{ .MarshalHCL }}`, `{{ sql . }}`, and `{{ mermaid . }}`. The OSS `--exclude` flag filters inspected resources with Atlas-style globs and `[type=...]` selectors, including the Atlas-documented `*[type=extension].version` field selector. `--include` is Atlas Pro-only and outside Ptah's OSS drop-in target. Other field-level exclude selectors, split/write templates, and dev-database inference remain explicit gaps. |

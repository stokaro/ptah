...
| `ptah atlas migrate status` | Forwards to `ptah migrations status` with Atlas revision-table metadata and Atlas-format migration directories by default; supports `--dir-format atlas` and `--revisions-schema`. |
...
| `ptah atlas migrate set` | Forwards to `ptah migrations repair` with Atlas revision-table metadata and Atlas-format migration directories by default; supports `--dir-format atlas` and `--revisions-schema`. |

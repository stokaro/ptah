...
about a reverse plan that is computed for the run as a whole. Each rollback line
is `--rollback <SQL>`, which Liquibase runs, where the community binary writes
`--rollback: <SQL>`, which Liquibase reads as a comment.
...
or one whose desired schema names a schema the pinned `--from` URL does not
cover. The shape cannot record either, and without the second the stale-plan
check would not read that schema.
...
Native twin: [`ptah db drop-all`](../native-commands/), which drops every schema
object in the database. It registers no `--include` or `--exclude`, so the
narrowed cleanup this command performs is reachable only here.

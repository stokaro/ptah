...
| `ptah migrations import` | Convert a golang-migrate, Goose, Flyway, or Liquibase migration directory into Ptah's native format, auto-detecting the source tool unless `--from` is set. |

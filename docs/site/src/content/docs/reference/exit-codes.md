...
| `ptah inference plan` | Plan printed, including a plan that is blocked. | Not used. | Usage error, connection failure, specification failure, schema-read failure, or a record that could not be left where `--publish-evidence` or `--evidence-file` named. Recording is the whole effect of those flags on this verb, so a record it did not leave is a run that did nothing. Both destinations are attempted and both are reported. |

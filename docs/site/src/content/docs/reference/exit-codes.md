...
| `ptah migrations repair` | Migration revision repaired, or dry-run output printed. | Not used. | Usage error, connection failure, revision lookup failure, a dirty revision that recorded no applied statement and so has nothing to finish, or repair failure. |

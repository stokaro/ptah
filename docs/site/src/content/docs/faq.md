...
## Choosing a workflow {#g-workflow}
...
additional ways in, not a condition of use.
...
migrations have to stay part of the project history.
...
generation needs no production access.

- [Generate migrations](../versioned/generate/#generate-without-the-target-database-replay)
...
## Two binaries: ptah and ptah-compat {#g-binaries}

One question behind three symptoms: the Atlas-shaped surface and the native surface are separate on purpose.
...
## Coming from another tool {#g-tools}
...
desired-schema workflow.

- [Import from another tool](../versioned/import/#a-repeatable-becomes-a-one-time-migration)
...
Partly. `ptah-compat` presents an Atlas-compatible surface for a gradual move. Full
...
- [Atlas compatibility](../atlas/overview/)
- [Migrate from Atlas](../atlas/adoption/)

## Schema sources and scope {#g-scope}
...
still has to be agreed between those parts.
...
Yes, for constructs both engines can express. A shared description does not make
engine capabilities equal, so check the rendering and the capability profile for
each target and keep engine-specific parts separate.
...
that the target engine supports, or reconsider the engine.

- [Troubleshooting](../operate/troubleshooting/#a-dialect-capability-is-unsupported)
...
versioned migration and test it against the target engine.
...
## Migration history {#g-history}

- [Troubleshooting](../operate/troubleshooting/#database-connection-fails)
...
continues on its normal chain.

- [Checkpoints](../versioned/checkpoints/#how-a-checkpoint-applies)

### After creating a checkpoint, can I delete the earlier migrations? {#delete-old-migrations}

No. Do not read a checkpoint as permission to delete history. Existing databases may
...
No. A checkpoint captures the resulting schema, not table contents. Data a new
...
- [Maintain migration history](../versioned/maintain-history/#move-a-migration-to-the-end-rebase)

### Rebase was allowed against my database. Does that make it safe? {#rebase-allowed-not-safe}

Partly. The check speaks for the database you named; it does not prove
...
## Seed and reference data {#g-data}
...
## Rollback and failures {#g-failures}

### Can I roll a checkpoint-built database back to a point inside the old history? {#checkpoint-rollback-boundary}

Partly. Not by rolling that history back: those individual migrations never ran on this
...
schema against the application version you are running.
...
check the execution mode and the actual state of the database first.

- [Apply migrations](../versioned/apply/#execution-controls)
...
- [Maintain migration history](../versioned/maintain-history/#repair-a-dirty-revision-state)
...
the database first, then reconcile the metadata with it.
...
transaction; it cannot grant the engine a capability it lacks.

- [Apply migrations](../versioned/apply/#what---tx-mode-all-cannot-carry)
...
blocking policy on explicitly.

- [Generate migrations](../versioned/generate/#failure-modes)
...
Yes. Use pre-migration assertions for conditions you can check in the data. They
...
execution, and that needs a disposable database.
...
build and review a new plan.

- [Apply a desired schema](../direct/apply/#failure-modes)
...
other SQL from a client that ignores it.

- [Apply migrations](../versioned/apply/#execution-controls)

## Reading a check result {#g-results}

Status, compare and drift answer three different questions; exit codes 1 and 2 separate a finding from a failure.
...
No. Migration status answers a question about recorded history, not about whether the
live schema matches an expectation. For the second question use `ptah schema
drift` with the desired state.

- [Apply migrations](../versioned/apply/#check-status)
- [Compare and drift](../direct/compare-and-drift/)
...
Partly. It matches within the check you selected, not across the database. Review the
...
Yes, it is drift. A difference does not stop existing because no supported planning
...
keep. Do not use it to paper over a change to shared history.
...
No. On the native surface a missing integrity file is allowed, while a file that
exists is verified. Use `--verify-sum` where the file has to be present and
checked.

- [Integrity and safety](../versioned/integrity-and-safety/#a-directory-that-was-never-hashed)

## Delivery and registries {#g-delivery}
...
- [OCI registry artifacts](../operate/oci-registry/#authenticate-securely)
...
No. A tag is a mutable pointer. Promote one verified digest between environments
...
- [OCI registry artifacts](../operate/oci-registry/#identity-integrity-and-authenticity)
...
a package does not make it a stable public contract.
...
- [Configure agent permissions](../operate/ai-agent-permissions/)
...
## Inference migrations {#g-inference}

### I want to change embedding model without rewriting every live vector at once. Can Ptah help? {#change-embedding-model}

Yes. For PostgreSQL with pgvector, use inference migrations: a new generation is built
...
- [Inference migrations](../inference/overview/#who-does-what)
...
rather than deleting the old data by hand.

- [Inference migrations](../inference/overview/#what-a-migration-looks-like)

## Kubernetes {#g-kubernetes}
...
documentation, releases, and support matrix.

- [Kubernetes operator](../operate/kubernetes-operator/)

### We use versioned migrations. Can the operator run `up` for us? {#operator-and-versioned-migrations}

No. `PtahSchema` reconciles a desired schema, and no resource applies a

...
Ptah reads the desired schema from SQL, YAML or HCL files, from annotated Go
structs, and from an external loader. Every one of them parses into the same
internal representation before anything is planned or rendered, so the
operations on this page work the same way whichever you keep.
...
Both flags repeat, and they mix: `--schema-file` twice and `--root-dir` beside
it is one composite desired schema.
...
schema, and reports the first structural problem it finds otherwise. That is
the cheapest check to put in front of a review: it needs no server and no
migration directory.

## Next steps

- The syntax and limits of each source: [SQL](../sql/), [YAML](../yaml/),
  [HCL](../hcl/), [Go annotations](../go-annotations/), and

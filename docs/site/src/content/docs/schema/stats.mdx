...
export const statsSample = `${import.meta.env.BASE_URL}samples/schema-stats.openmetrics.txt`;
...
Ptah emits metrics text and exits; it does not ship a dashboard or metrics
server. <a href={statsSample} download>Download the real OpenMetrics output</a>
generated from the committed SQLite fixture, then feed that text to the
collector and visualization system you already operate.

...
- `--to hcl` with `--schema-file`. That target rewrites the Go files it reads —
  ...
  its source is `--root-dir`. Cleanup refuses outright when the annotations
  carry [export-only naming metadata](#names-in-the-contract), because HCL
  cannot preserve it. Converting a schema file to HCL is a different operation
...
Ptah can export SQL, YAML, HCL, DBML, Go, OCI, and composite desired schemas,
but only Go annotations can author export-only naming metadata. The other
accepted sources still produce OpenAPI, GraphQL, and Protobuf contracts; their
published names are derived from the persistence names. Broader authoring
support is tracked in
[`stokaro/ptah#2607`](https://github.com/stokaro/ptah/issues/2607).

| Desired-schema source | Naming behavior |
| --- | --- |
| SQL, YAML, HCL, or DBML file | Uses derived names; these formats have no `api_name` or target-specific name syntax. |
| Go annotations | Can declare `api_name`, `openapi_name`, `graphql_name`, and `proto_name`. |
| OCI artifact | Uses the names in its canonical HCL payload, which cannot carry export-only names; publishing a schema that declares them is refused. |
| Composite source | Preserves overrides on objects loaded from Go; a static source cannot add or overlay them. |
| External or configured loader | Is not an input to `schema export`; materialize its SQL, HCL, or YAML output first, which then uses derived names. |

:::caution[HCL cannot carry these declarations, and refuses rather than drop them]
HCL has no spelling for `api_name`, `api_type`, `api_expose` or the
target-specific names, so `ptah schema export --to hcl` reports each one it
would lose:

```text
export metadata api_name="Account" is not represented in HCL
export metadata api_expose="read" is not represented in HCL
```

`--cleanup-go-annotations` refuses on that report, exits 2, and leaves the Go
sources untouched, so the annotations holding the contract are never the only
casualty of a conversion. Publishing an OCI artifact refuses for the same
reason, before anything is pushed.

Keep the annotated Go source and export contracts directly from it. A static
spelling for this metadata is
[`stokaro/ptah#2607`](https://github.com/stokaro/ptah/issues/2607).
:::

The examples in this section therefore use Go annotations. There is no
equivalent static-file spelling to substitute. Declare `api_name` on the table
or field directive when the persistence and API identities should differ:
...
`amount_minor`. Go annotations provide one shared attribute and three
target-specific attributes on both the field and table directives:

| Attribute | Export target |
| --- | --- |
| `api_name` | Shared fallback for OpenAPI, GraphQL, and Protobuf. |
| `openapi_name` | OpenAPI only. |
| `graphql_name` | GraphQL only. |
| `proto_name` | Protobuf only. |

```go
//ptah:schema:table name="billing_invoices" api_name="invoices" openapi_name="invoice_documents" graphql_name="Invoice" proto_name="invoice_records"
type Invoice struct {

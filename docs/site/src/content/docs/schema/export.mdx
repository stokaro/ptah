...
- `--to hcl` with `--schema-file`. That target rewrites the Go files it reads —
  ...
  its source is `--root-dir`. Do not use cleanup when the annotations contain
  [export-only naming metadata](#names-in-the-contract), because HCL cannot
  preserve it. Converting a schema file to HCL is a different operation from
  migrating annotations out of Go code.
...
| OCI artifact | Uses the names in its canonical HCL payload, which cannot carry export-only names. |
...
:::danger[Do not clean or publish these declarations through HCL]
`ptah schema export --to hcl --cleanup-go-annotations` writes HCL without
export-only metadata and then removes the Go annotations that held it. OCI
capture uses the same canonical HCL representation, so an OCI artifact also
loses these declarations. Keep the annotated Go source and export contracts
directly from it until [`stokaro/ptah#2607`](https://github.com/stokaro/ptah/issues/2607)
is resolved.
:::

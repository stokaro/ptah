...
Ptah currently exports lineage as text or JSON, not as a graph. The committed
fixture produces two resolved view edges and one explicitly undecided view;
download the <a href={lineageText} download>real text output</a> or
<a href={lineageJSON} download>real JSON output</a>. Graph export is tracked in
[issue #2576](https://github.com/stokaro/ptah/issues/2576).
...
<ProductPreview
  ...
  caption="Explanatory diagram—not output from `ptah schema lineage`—of the real text and JSON fixture above."

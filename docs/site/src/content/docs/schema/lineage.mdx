...
import lineageExplanation from '../../../assets/schema-lineage-explanatory.svg';

export const lineageText = `${import.meta.env.BASE_URL}samples/schema-lineage.txt`;
export const lineageJSON = `${import.meta.env.BASE_URL}samples/schema-lineage.json`;
export const lineageDOT = `${import.meta.env.BASE_URL}samples/schema-lineage.dot`;
...
Ptah exports lineage as text, JSON, or a Graphviz digraph with `--format dot`.
The committed fixture produces two resolved view edges and one explicitly
undecided view; download the <a href={lineageText} download>real text
output</a>, <a href={lineageJSON} download>real JSON output</a>, or
<a href={lineageDOT} download>real DOT output</a>.

The graph draws an unresolved view dashed and carries its reason as a tooltip
rather than leaving it out, and it sorts every node and edge so the same schema
renders byte-identically twice — which is what makes it a review artifact a
repository can commit and diff.

<ProductPreview
  id="schema-lineage-explanatory"
  src={lineageExplanation}
  alt="An explanatory dependency diagram where orders.customer_id feeds order_totals.buyer, orders.total_cents feeds order_totals.cents, and customer_orders remains undecided because its columns come from a join."
  caption="Explanatory diagram—not output from `ptah schema lineage`—of the same fixture. The DOT download above is the real graph; this drawing is here because a rendered picture needs a Graphviz the documentation build does not carry."
  notice="Resolved edges remain separate from the undecided view, because treating an unresolved join as no dependency would make a destructive change look safe."
  fullSizeHref={lineageExplanation.src}
  downloadHref={lineageExplanation.src}
  sourceHref={lineageExplanation.src}
  reproduce="ptah schema lineage --schema-file docs/site/fixtures/product-output/lineage.sql --dialect sqlite --format json"
/>
...
`undecided` as well, and the reason says which of the two happened:
...
Four statements land in the second row: an `EXECUTE`, which composes its

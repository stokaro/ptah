...
Found 1 table(s), 7 field(s), 1 enum(s)
...
The output passes `buf lint` with the `STANDARD` rule set: message names are
PascalCase, field names are `lower_snake_case`, and every enum has the required
zero value under its own `ENUM_NAME_` prefix.
...
| `output file was modified since it was generated: recorded …, computed …` | The `.proto` was hand-edited. Revert it, then change the Go model instead. |
...
| `output file is not valid protobuf: …` | The committed file no longer parses, usually after a bad merge. |
...
- Two tables that map to the same message name fail the export naming both.
  ...
  a numeric suffix, because the result would depend on table order.

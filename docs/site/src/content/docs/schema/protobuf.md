...
description: Export a desired schema to a Protobuf Edition 2023 definition whose field numbers stay wire compatible as the schema changes.
type: how-to
audience:
  - "schema-author"
...
by `--schema-file`, or Go annotations under `--root-dir` — into a Protobuf definition:
...
Prerequisites: a built `ptah` binary and a desired schema as a YAML, HCL, SQL,
or DBML file or a directory of Go annotations. `buf` and `protoc` are needed
only if you want to lint or compatibility-check the result yourself.
...
Go annotations use `--root-dir`, exactly as they do on

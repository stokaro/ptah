...
A file written by an older Ptah carries the same three lines at the top and
declares `ptah:protobuf-export-version=1`. It is migrated, not refused: the next
export keeps every field and enum-value number, rewrites the file with the
header at the foot, and reports what it did.

```text
warning: ./proto/acme/inventory/v1/schema.proto: previous export uses format version 1, which carried the generated header at the top of the file; rewriting it as format version 2 with the header at the bottom, where protoc-gen-go does not copy it into the generated .pb.go
```

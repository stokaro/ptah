...
export const securitySamples = `${import.meta.env.BASE_URL}samples/visualize/`;
...
The live-database command prints the complete report. When you need to locate
the same class of findings on an entity diagram, `ptah viz --security` applies
the rules to Go-annotated models and marks the table nodes it can attach to:

<ProductPreview
  id="schema-security-visual"
  src={securityLight}
  darkSrc={securityDark}
  alt="A four-table schema diagram where orders has warning findings PRV01 and PRV03, products has informational PRV01, and a comment records a routine finding with no table node."
  caption="Real `ptah viz --security` output connects findings to tables without hiding findings that have no table node."
  notice="Orders shows two warning codes, products shows an informational code, and the DOT comment retains the SECURITY DEFINER routine finding; the command report remains the complete answer."
  fullSizeHref={securityLight.src}
  downloadHref={`${securitySamples}schema-security.svg`}
  sourceHref={`${securitySamples}schema-security.dot`}
...
Amber is `warning`, blue is `info`, and neutral nodes have no attached finding.
See [Visualize the schema](../visualize/#mark-where-the-security-findings-are)
for the Mermaid limitation and the complete diagram-output contract.

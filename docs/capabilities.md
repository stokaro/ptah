...
- **Direct migration consumption** — `ptah migrations up`, `status`, and `down` accept `oci://` through `--migrations-dir`. `--verify-sum` on `push`, `up`, `down` and `status` requires the directory to carry a sum and to match it; it verifies the pulled directory against the sum that traveled inside it, so over a movable tag it proves internal consistency only, and each of those verbs prints the resolved digest and its `@sha256:` pin whenever the check ran over a tag. Pin a digest for authenticity.

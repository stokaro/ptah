...
**Atlas-compatible Ptah surface.** `ptah-compat migrate test` and `ptah-compat schema test` forward to the native runners with Atlas-shaped flags (`--dir`/`-u --url`, `--dev-url`, `--run`, project flags) and the native exit-code contract. `schema test -u` accepts a directory of Go schema annotations, a `.sql` or `.hcl` schema file, or a database URL whose live schema is introspected. HCL files take explicit `--var` values, while project-selected `data.hcl_schema` files keep their block-scoped `vars`. A database source must share the dialect of the throwaway database. Roles and grants from introspection are dropped before application, with the omission reported on stderr so stdout carries only the report.

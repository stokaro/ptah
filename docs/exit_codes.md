...
| `ptah migrations status` | Status printed, including pending migrations by default. | The database is not up to date with the directory when `--exit-code` is set: pending migrations exist, or an applied migration's file no longer accounts for the checksum the database recorded. | Usage error, connection failure, migration directory error, or status-read failure. |

...
- PostgreSQL `function` blocks with `schema`, `lang`, `arg`, `return`,
  `return_set`, `return_table`, `security`, `volatility`, `leakproof`,
  `parallel`, `set`, `as`, and `comment`; `return_set = true` renders `RETURNS
  SETOF <type>` and `return_table = { a = integer, b = text }` renders `RETURNS
  TABLE(a integer, b text)`, keeping the columns in the order they were written
  because that order is the shape of the row the function returns. Declaring
  `return` beside `return_table`, or `return_set` beside it, is refused: each
  pair is a syntax error on the server too; the Ptah `params` parity
...
- `procedure` blocks, taking every attribute `function` takes except `return`,
  `leakproof`, `parallel`, `return_set` and `return_table`, each of which is
  refused inside one. Measured on
...
`schemamodel.Database` IR. These blocks are primarily useful with
PostgreSQL-family rendering.
...
- grantor metadata
- function options outside Ptah's current IR, such as argument defaults.

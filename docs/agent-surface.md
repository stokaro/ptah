...
| `project adopt` | reads | none | — | classifies every construct a project file declares as exact, compat-only or unsupported; --check reports that and writes nothing, while the bare verb rewrites the compat-only spellings in the file and refuses a project declaring anything unsupported; --preflight additionally reads the revision history in the database the project points at, and writes nothing there -- not the revision table it looks for, and not the layout of one it finds |

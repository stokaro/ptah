...
| `project adopt` | none | none | — | classifies every construct a project file declares as exact, compat-only or unsupported and reports whether the project is native-ready; --check is required, it opens no database and it writes no file |
...
| `project adopt` | classifies every construct a project file declares as exact, compat-only or unsupported and reports whether the project is native-ready; --check is required, it opens no database and it writes no file |

	...
	tests := []internalDirectiveCase{
		{
			...
			observe: func(sql string) (bool, error) {
				timeouts, err := ParseTimeouts(sql)
...
func TestMisplacedTxtarDirectiveIsRefusedRatherThanDropped(t *testing.T) {
	...
	_, _, err := ParseAtlasTxtar("1_x.sql", sql)
...
func TestHashCommentHeaderFollowsTheLexerRatherThanAList(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "postgres", dialect: platform.Postgres, wantMode: FileTxModeNone},
		{name: "mysql", dialect: platform.MySQL, wantMode: FileTxModeNone},
		{name: "mariadb", dialect: platform.MariaDB, wantMode: FileTxModeNone},
		{name: "sqlite", dialect: platform.SQLite, wantMode: FileTxModeNone},
		{name: "clickhouse", dialect: platform.ClickHouse, wantMode: FileTxModeNone},
		{name: "cockroachdb", dialect: platform.CockroachDB, wantMode: FileTxModeNone},
		{name: "yugabytedb", dialect: platform.YugabyteDB, wantMode: FileTxModeNone},
		{name: "spanner", dialect: platform.Spanner, wantMode: FileTxModeNone},
		{
			...
			wantMode:      FileTxModeUnspecified,
			wantMisplaced: []int{2},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			parsed := ParseFileTxModeForDialect("1_x.sql", sql, test.dialect)
	...
	for _, test := range tests {
		for _, dialect := range targets {
			t.Run(test.name+" in dialect "+dialect, func(t *testing.T) {
				...
				parsed := ParseFileTxModeForDialect("1_x.sql", test.sql, dialect)
				timeouts, err := ParseTimeoutsForDialect(test.sql, dialect)
				...
				c.Check(parsed.Err, qt.IsNil)
				c.Check(parsed.Mode, qt.Equals, FileTxModeNone)
...
func TestDashDashWithNoBoundaryStillEndsTheMySQLHeader(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			timeouts, err := ParseTimeoutsForDialect(sql, test.dialect)
...
func TestHashCommentDoesNotWidenTheAtlasHeader(t *testing.T) {
	...
	for _, dialect := range []string{"", platform.MySQL, platform.ClickHouse} {
		t.Run("dialect "+dialect, func(t *testing.T) {
			...
			c.Check(mode, qt.Equals, FileTxModeUnspecified)
...
func TestMalformedHeaderDirectiveKeepsItsOwnDiagnosis(t *testing.T) {
	...
	got := ParseFileTxMode("1_x.sql", "-- +ptah no_transaction=maybe\n\n"+directiveStatement)

	c.Check(got.Err, qt.ErrorMatches, `invalid \+ptah no_transaction value "maybe": expected true or false`)
	c.Check(got.Source, qt.Equals, FileTxModeSourcePtah)
}
...
func TestAtlasHeaderBoundaryMatchesTheParserThatUsesIt(t *testing.T) {
	for _, placement := range directiveplacement.All {
		t.Run(placement.Name, func(t *testing.T) {
			...
			c.Check(has && mode == FileTxModeNone, qt.Equals, insideBlock,
				qt.Commentf("source:\n%s", sql))
		})
	}
}

...
// RLSPolicies performs PostgreSQL RLS policy comparison between the desired and current schemas.
//
...
func RLSPolicies(
	...
	RLSPoliciesWithSemantics(desired, current, diff, identifier.ForDialect(""), cov, nil)
}
...
func RLSPoliciesWithSemantics(
	...
	databasePolicyMap := make(map[tableMemberKey]catalog.RLSPolicy, len(current.RLSPolicies))
	for _, rlsPolicy := range current.RLSPolicies {
...
// RLSEnabledTables performs RLS enablement comparison between the desired and current schemas.
//
...
// RLSPolicyDefinitions performs detailed comparison between the desired and current RLS policy definitions.
//

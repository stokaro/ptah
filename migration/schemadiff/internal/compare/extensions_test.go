...
func TestExtensions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Run the comparison
			compare.Extensions(desired, current, diff, nil, compare.CoverageOf(desired, current))
...
func TestExtensionsWithSemanticsComparesInstallationSchema(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			generated: schemamodel.Extension{Name: "pgcrypto", Schema: "extensions"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			c := qt.New(t)
			diff := &difftypes.SchemaDiff{}
			desired := &schemamodel.Database{Extensions: []schemamodel.Extension{test.generated}}
			current := &catalog.Database{Extensions: []catalog.Extension{test.database}}
			compare.ExtensionsWithSemantics(
				desired,
				current,
				diff,
				nil,
				compare.CoverageOf(desired, current),
...
func TestExtensions_EdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectPanic {
				c.Assert(func() {
					compare.Extensions(tt.desired, tt.current, diff, nil, compare.CoverageOf(tt.desired, tt.current))
				}, qt.PanicMatches, ".*")
			} else {
				// Should not panic
				compare.Extensions(tt.desired, tt.current, diff, nil, compare.CoverageOf(tt.desired, tt.current))
...
func TestExtensions_WithIgnoreConfiguration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			compare.Extensions(desired, current, diff, tt.options, compare.CoverageOf(desired, current))

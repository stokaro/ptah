...
func TestSameColumnNames_ComparesUnderTheDialectsRule(t *testing.T) {
	tests := []struct {
		...
		// MySQL folds ASCII column case. Measured on mysql:8.4.11 and
		// mariadb:11.8.9: a table declaring `A` and `a` answers ERROR 1060
		// Duplicate column name 'a', and a foreign key on `a` binds to a column
		// declared `A` and reuses its key. So a desired `author_id` and a
		// catalog AUTHOR_ID are one column, and comparing the spellings reports
		// drift on every run (stokaro/ptah#2771). PostgreSQL above is the row
		// that keeps the fold from spreading.

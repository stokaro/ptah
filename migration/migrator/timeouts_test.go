...
func TestMigrationTimeoutsUseTheExecutionDialect(t *testing.T) {
	...
	c.Assert(directiveRegion(sql, platform.MySQL), qt.Equals,
		"# generated by deploy tooling\n-- +ptah lock_timeout=3s statement_timeout=30s\n")
	loaded, err := parseMigrationTimeoutDirectives(sql)
	c.Assert(err, qt.IsNil)
	c.Assert(loaded.IsZero(), qt.IsTrue)
	...
	postgres, err := migration.upTimeoutsForDialect(platform.Postgres)
	c.Assert(err, qt.IsNil)
	c.Assert(postgres.IsZero(), qt.IsTrue)
}

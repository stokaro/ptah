...
//
// A failed up and a failed down otherwise write byte-identical rows -- same
// state, same shape of applied/total -- so nothing downstream can tell which
// body stopped halfway. `migrations repair --resume-from` then replays the up
// body over a partially reverted schema and records the migration applied
// (stokaro/ptah#995).
//
...
// answering the same question on every dialect, with no DDL and no rewrite of
// existing rows.

...
func TestMigrationMetadataSQL_UsesPlaceholders(t *testing.T) {
	...
	queries := map[string]struct {
		...
		"complete migration": {sql: (&Migrator{}).completeMigrationSQL(), placeholders: 7},
		"checkpoint":         {sql: (&Migrator{}).checkpointMigrationSQL(), placeholders: 6},
		"begin rollback":     {sql: (&Migrator{}).beginRollbackSQL(), placeholders: 6},
		"fail migration":     {sql: (&Migrator{}).failMigrationSQL(), placeholders: 8},

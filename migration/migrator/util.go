...
// [DiscoverMigrationFiles] itself, so its coverage follows the selection instead
// of constraining it.
func atlasSumGovernsSelection(format MigrationDirFormat, hasAtlasSum bool) bool {
	if format == MigrationDirFormatPtah {
		return false
	}
	return format == MigrationDirFormatAtlas || hasAtlasSum
}

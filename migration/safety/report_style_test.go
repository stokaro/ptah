...
func TestRenderHTML_FetchesNothing(t *testing.T) {
	...
	c.Assert(regexp.MustCompile(`(?i)src\s*=\s*"(https?:)?//`).FindAllString(page.String(), -1), qt.HasLen, 0)
	...
	c.Assert(regexp.MustCompile(`(?i)<a [^>]*href="https?://`).FindAllString(page.String(), -1), qt.HasLen, 1)
}

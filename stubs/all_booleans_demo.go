...
type BooleanDemo struct {
	...
	//migrator:schema:index name="idx_boolean_demo_email" fields="email"
	...
	//migrator:schema:field name="special_code" type="VARCHAR(50)" unique not_null
	//migrator:schema:index name="idx_boolean_demo_special_code" fields="special_code"
	SpecialCode string `db:"special_code"`

	// Mixed quoted and bare canonical syntax
	//migrator:schema:field name="status" type="VARCHAR(20)" not_null="true" unique default="pending"
	//migrator:schema:index name="idx_boolean_demo_status" fields="status"

...
// that needs stdin held open is
// TestPtahLSVersionAnswersWithStdinHeldOpen below.
//
...
// nothing and exited".
//
// wantStdout and wantStderr are anchored patterns, so an empty one says the
// stream carried nothing at all.
func TestPtahLSArgumentHandling(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:       []string{"version"},
			wantExit:   0,
			wantStdout: versionBlockPattern,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Assert(stdout.String(), qt.Matches, tt.wantStdout)
			c.Assert(stderr.String(), qt.Matches, tt.wantStderr)
		})
	}
}

// TestPtahLSVersionAnswersWithStdinHeldOpen is the defect of the table above
// seen from its worse side. Pre-fix this does not fail, it hangs: the process
// is still blocked in the server's read loop when the budget expires, which is
// why the answer has to be measured against a stdin that never reaches EOF
// rather than against the /dev/null every row above gets.
func TestPtahLSVersionAnswersWithStdinHeldOpen(t *testing.T) {
	c := qt.New(t)
	binPath := buildPtahLSBinary(c)
	...
	run := newPtahLSProcess(ctx, binPath, "version")
	run.Stdin = heldOpenStdin(c)
	var stdout, stderr bytes.Buffer
	run.Stdout = &stdout
	run.Stderr = &stderr
	...
	c.Assert(ctx.Err(), qt.IsNil,
		qt.Commentf("process did not exit within %s", commandBudget))
	c.Assert(run.ProcessState.ExitCode(), qt.Equals, 0,
		qt.Commentf("stdout: %q stderr: %q", stdout.String(), stderr.String()))
	c.Assert(stdout.String(), qt.Matches, versionBlockPattern)
...
// usageErrorPattern pins the diagnostic line this binary owns byte-exactly and
...
func usageErrorPattern(wantFirstLine string) string {
	return `(?s)` + wantFirstLine + `Usage of ptah-ls:\n.*`
}

...
func TestCompatMigrateApply_FlywayRecordsSourceIdentityWithoutChangingOrder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			c := qt.New(t)
			dir := writeHashedFlywayDir(c.TB, test.migrations)
			dbPath := filepath.Join(filepath.Dir(dir), "apply.db")

			stdout, stderr, err := compatApplyConverted(dir, "flyway", dbPath)

			c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
			c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, test.wantRows)
			for _, version := range test.wantRows {
				c.Assert(revisionOperatorVersion(c.TB, dbPath, version), qt.Equals, "Ptah/source-identity")
			}
		})
	}
}
...
`,
	...
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"owned_empty_partial"})

	writeAtlasApplyProjectMigration(c.TB, dir, "V.sql", `CREATE TABLE owned_empty_partial (id INTEGER PRIMARY KEY);
...
`)
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"owned_empty_partial", "owned_empty_retry"})
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{""})
}
...
`,
	...
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"retired_partial"})
...
FROM atlas_schema_revisions WHERE version = ?`, retiredIdentity).Scan(
	...
	writeAtlasApplyProjectMigration(c.TB, dir, "Vsurviving__later.sql",
	...
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"retired_partial"})
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{retiredIdentity})
...
func assertCompatFlywayRemovedHistoryRemainsReadable(
	...
	stdout, stderr, err := compatApplyConverted(dir, "flyway", dbPath)
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	insertCompatAtlasCloudIdentifier(c.TB, dbPath)
	c.Assert(os.Remove(filepath.Join(dir, retiredFile)), qt.IsNil)
	hashConvertedApplyDir(c.TB, dir, "flyway")
	...
	writeAtlasApplyProjectMigration(c.TB, dir, olderFile,
	...
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{table})
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{compatAtlasCloudIdentifierVersion, retiredToken})
}

func insertCompatAtlasCloudIdentifier(tb testing.TB, dbPath string) {
	c := qt.New(tb)
	c.Helper()
	db, err := sql.Open("sqlite", dbPath)
	c.Assert(err, qt.IsNil)
...
func TestCompatMigrateApply_FlywayBaselineResolvesExactToken(t *testing.T) {
	...
	c.Assert(userTables(c.TB, dbPath), qt.HasLen, 0)
}
...
func TestCompatMigrateApply_FlywayBaselineSettlesSameTokenSquash(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"after_same_token_baseline"})
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{"2", "3"})
	c.Assert(revisionType(c.TB, dbPath, "2"), qt.Equals, 1)
	c.Assert(revisionOperatorVersion(c.TB, dbPath, "2"), qt.Equals, "Ptah/source-baseline")
...
func TestCompatMigrateApply_FlywayBaselineDoesNotSettleALaterSameTokenBaseline(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(revisionType(c.TB, dbPath, "2"), qt.Equals, 1)
	c.Assert(revisionOperatorVersion(c.TB, dbPath, "2"), qt.Equals, "Ptah/source-identity")
...
func revisionOperatorVersion(tb testing.TB, dbPath, version string) string {
	c := qt.New(tb)
	c.Helper()
	db, err := sql.Open("sqlite", dbPath)
	c.Assert(err, qt.IsNil)
...
func TestCompatMigrateApply_FlywayBaselinePreparationErrorUsesExactToken(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	markAtlasRevisionDirty(c.TB, dbPath, "1.5", 3)
...
func TestCompatMigrateApply_FlywayToVersionResolvesExactToken(t *testing.T) {
	...
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{"1", "1.5"})
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"exact_bound_half", "exact_bound_one"})
}
...
func TestCompatMigrateApply_FlywayToVersionTreatsExactEmptyTokenAsPresent(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"empty_target"})
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{""})
}
...
func TestCompatMigrateApply_FlywayRepeatableBodyChangeRemainsSettled(t *testing.T) {
	...
	stdout, stderr, err := compatApplyConverted(dir, "flyway", dbPath)
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	writeAtlasApplyProjectMigration(c.TB, dir, "R__only.sql",
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(stdout, qt.Contains, "No migration files to execute")
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"repeatable_settled"})
}
...
func TestCompatMigrateApply_FlywayDotPrefixedIdentityRemainsVisible(t *testing.T) {
	...
	stdout, stderr, err := compatApplyConverted(dir, "flyway", dbPath)
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(revisionRows(c.TB, dbPath), qt.DeepEquals,
...
func TestCompatMigrateApply_FlywayRolledBackFailureLeavesNoExactDirtyRow(t *testing.T) {
	...
	c.Assert(err, qt.IsNotNil)
	c.Assert(revisionVersions(c.TB, dbPath), qt.HasLen, 0)

	writeAtlasApplyProjectMigration(c.TB, dir, "V1.5__retry.sql",
		"CREATE TABLE rolled_back_exact (id INTEGER PRIMARY KEY);\n")
	hashConvertedApplyDir(c.TB, dir, "flyway")
	stdout, stderr, err := compatApplyConverted(dir, "flyway", dbPath)
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{"1.5"})
}
...
func TestCompatMigrateApply_FlywayTokenEndingRIsNotAtlasRepeatable(t *testing.T) {
	...
	writeAtlasApplyProjectMigration(c.TB, dir, "V1R__ordinary.sql",
	...
	c.Assert(userTables(c.TB, dbPath), qt.DeepEquals, []string{"token_ending_r"})
}
...
func TestCompatMigrateApply_ExactFlywayIdentityDoesNotNarrowOtherFormats(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	rewriteRevisionVersion(c.TB, dbPath, "1", "00001")

	stdout, stderr, err = compatApplyConverted(dir, "goose", dbPath)
	c.Assert(err, qt.IsNil, qt.Commentf("stdout:\n%s\nstderr:\n%s", stdout, stderr))
	c.Assert(stdout, qt.Contains, "No migration files to execute")
	c.Assert(revisionVersions(c.TB, dbPath), qt.DeepEquals, []string{"00001"})
}

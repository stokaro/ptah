	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for name, content := range tt.files {
				atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, name, content)
			}
			...
			c.Assert(atlastest.SqliteTableCount(c, dbPath, "up_ran"), qt.Equals, 1)
			c.Assert(atlastest.SqliteTableCount(c, dbPath, "down_ran"), qt.Equals, 0)
		})
	}
}
...
func TestMigrateApplyFormatOutputRendersFromConvertedFilesystem(t *testing.T) {
	...
	dir := t.TempDir()
	migrationsDir := filepath.Join(dir, "migrations")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "1_init.sql",
		"-- +goose Up\nCREATE TABLE up_ran (id INTEGER PRIMARY KEY);\n-- +goose Down\nCREATE TABLE down_ran (id INTEGER PRIMARY KEY);")
...
func TestMigrateApplyFlywayMajorMinorVersionsExecuteInOrder(t *testing.T) {
	...
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1.5__create.sql", "CREATE TABLE widgets (id INTEGER PRIMARY KEY);")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V2__extend.sql", "ALTER TABLE widgets ADD COLUMN label TEXT;")
...
func TestMigrateApplyGooseStatementBlockExecutesUpOnly(t *testing.T) {
	...
	c.Assert(err, qt.IsNil, qt.Commentf("command output:\n%s", out.String()))
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "up_ran"), qt.Equals, 1)
	c.Assert(atlastest.SQLiteRowCount(c, dbPath, "up_ran"), qt.Equals, 1)
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "down_ran"), qt.Equals, 0)
}
...
func TestMigrateApplyRejectsFlywayVersionCollisionBeforeOpeningDatabase(t *testing.T) {
	...
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1__a.sql", "CREATE TABLE a (id INTEGER PRIMARY KEY);")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1__b.sql", "CREATE TABLE b (id INTEGER PRIMARY KEY);")
...
func TestMigrateApplyRejectsDuplicateConvertedVersionBeforeOpeningDatabase(t *testing.T) {
	...
	dir := t.TempDir()
	migrationsDir := filepath.Join(dir, "migrations")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "1_init.sql", "-- +goose Up\nCREATE TABLE a (id INTEGER PRIMARY KEY);")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "01_init.sql", "-- +goose Up\nCREATE TABLE b (id INTEGER PRIMARY KEY);")
	hashConvertedApplyDir(c, migrationsDir, "goose")
...
func TestMigrateApplyRejectsMissingUpDirectiveBeforeOpeningDatabase(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dir := t.TempDir()
			migrationsDir := filepath.Join(dir, "migrations")
			atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "1_init.sql", tt.content)
			hashConvertedApplyDir(c, migrationsDir, tt.format)
...
func TestMigrateApplyRejectsUnknownURLFormatBeforeOpeningDatabase(t *testing.T) {
	...
	dir := t.TempDir()
	migrationsDir := filepath.Join(dir, "migrations")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "1_init.sql", "CREATE TABLE never_created (id INTEGER PRIMARY KEY);")
...
func TestMigrateApplyFlywayMidSequenceInsertionKeepsStableVersions(t *testing.T) {
	...
	dir := t.TempDir()
	migrationsDir := filepath.Join(dir, "migrations")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1__base.sql", "CREATE TABLE t_v1 (id INTEGER PRIMARY KEY);")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1.5__minor.sql", "CREATE TABLE t_v15 (id INTEGER PRIMARY KEY);")
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V2__major.sql", "CREATE TABLE t_v2 (id INTEGER PRIMARY KEY);")
	hashConvertedApplyDir(c, migrationsDir, "flyway")
	dbPath := filepath.Join(dir, "apply.db")
	...
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "t_v1"), qt.Equals, 1)
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "t_v15"), qt.Equals, 1)
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "t_v2"), qt.Equals, 1)
	before := atlastest.SqliteAtlasRevisionVersions(c, dbPath)

	// Insert a migration that sorts in the middle (V1.6, between V1.5 and V2) and
	// re-apply.
	atlastest.WriteAtlasApplyProjectMigration(c, migrationsDir, "V1.6__hotfix.sql", "CREATE TABLE t_v16 (id INTEGER PRIMARY KEY);")
	...
	c.Assert(atlastest.SqliteTableCount(c, dbPath, "t_v16"), qt.Equals, 1)
	after := atlastest.SqliteAtlasRevisionVersions(c, dbPath)

...
//
// It is exported for the directory walker, which accumulates one Database
// across many files before merging it.
...
//
// It is exported for the directory walker, which appends each parsed file to
// the accumulator NewDatabase returned.
...
// slice of the inputs, and then runs the same strict finalization that
// goschema.ParseFS applies to the per-file schemas of one directory. Every
// named object kind uses its stable database identity, with
...
//
// Merge is the reusable form of the merge goschema.ParseFS performs internally,
// so it can assemble a composite desired-state schema from heterogeneous
// sources (several Go roots, or a mix of Go, YAML, and HCL) and feed the result
// to the existing render/compare/migrate paths unchanged.
//
// Inputs may be raw parser output or already-finalized schemas from
// goschema.ParseDir, goschema.ParseFS, or an OCI artifact. Post-expansion
// conflict validation and deduplication make repeated finalization idempotent.
...
// MergeAccumulated reconciles and finalizes a mutable accumulator that a
// caller assembled with NewDatabase and AppendDatabase.
//
// It differs from Merge in what it owns: Merge copies and reconciles its
// inputs first, so the caller's source schemas are never mutated, while this
// one finalizes the accumulator in place and is only correct for an
// accumulator nobody else holds. The directory walker builds exactly that,
// which is why this half of the pair is exported.

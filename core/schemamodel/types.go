...
// Database represents the complete schema a source declared.
//
// Every authoring front end produces one -- Go struct annotations, YAML, HCL,
// SQL files, DBML -- and so does a caller that builds it in memory. It
// aggregates all schema information the source carried, its relationships, and
// the dependency information migration generation needs.
//
...
type Table struct {
	...
	// when it named none.
	//
	// A constraint name is what a later ALTER TABLE ... DROP CONSTRAINT names,
	// what a hand-written migration refers to, and what an operator reads in an
	// error. Nothing carried it, so a declared `CONSTRAINT c_pk PRIMARY KEY (b)`
	// reached PostgreSQL as `t_named_pkey` and `--dry-run` reported the schema
	// synced afterwards -- for a composite key as well as a single-column one
	// (stokaro/ptah#2180).

...
//
// It is exported for the parser, which builds the graph for a single-file
// Database that never reaches Merge.
//
...
//
// Parsers that do not go through goschema.ParseDir still need the same derived
// metadata as Go annotations: deduplicated declarations, dependency maps,
// self-referencing foreign keys, and dependency-ordered tables/functions.
...
// NormalizeTableScopedNames rewrites every table-scoped reference to the
// qualified name of the table it belongs to.
//
// Constraints, indexes, RLS policies, RLS enablement records, grants, and
// triggers all name their host table, and an annotation may name it by Go
// struct or by a bare table name. Resolving those to one spelling is what lets
// a later stage pair two records that describe the same object.
//
// It is exported because the parser runs it before the model is complete:
// [go.5x5.cz/ptah/core/goschema.ParseFile] and its ParseSource sibling produce
// one file's Database, which never reaches Merge, so the normalization Finalize
// performs for a merged model has to be reachable from outside this package.
...
//
// It is exported for the parser and its tests, which expand embedded fields
// outside a merge.
//

...
// implication edges: materialized views and every object kind's comment
// require the object kind, and replace syntax requires triggers. The dependants
// come off together with the object so that the result is a set Validate
// accepts — a renderer must never be handed a set describing a target that
// cannot exist.
func objectKindDeniedCaps(key capability.Capability) capability.Capabilities {
	return capability.Postgres16().
		...
		With(capability.FunctionComments, false).

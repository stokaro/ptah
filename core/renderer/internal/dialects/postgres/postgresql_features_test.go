...
func TestPostgreSQLRenderer_VisitIndex_PostgreSQLFeatures(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Assert(legacyPostgresSQL(sql), qt.Equals, tt.expected)
		})
	}
}
...
func TestPostgreSQLRenderer_CreateDomain(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(legacyPostgresSQL(sql), qt.Equals, "CREATE DOMAIN script_column_domain.positive_int AS bigint CHECK (VALUE > 0);\n")
}
...
func TestPostgreSQLRenderer_CompleteSchema(t *testing.T) {
	...
	c.Assert(legacyPostgresSQL(tableSQL), qt.Contains, "tags JSONB")

	c.Assert(legacyPostgresSQL(ginIndexSQL), qt.Equals, "CREATE INDEX idx_products_tags ON products USING GIN (tags);\n")
	c.Assert(legacyPostgresSQL(partialIndexSQL), qt.Equals, "CREATE INDEX idx_active_products ON products (status) WHERE deleted_at IS NULL;\n")
	c.Assert(legacyPostgresSQL(trigramIndexSQL), qt.Equals, "CREATE INDEX idx_products_name_trgm ON products USING GIN (name gin_trgm_ops);\n")
}
...
func TestPostgreSQLRenderer_IdentityColumns(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(legacyPostgresSQL(result), qt.Equals, `-- POSTGRES TABLE: users --
CREATE TABLE users (
...
func TestPostgreSQLRenderer_PrimaryKeyInclude(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	c.Assert(legacyPostgresSQL(result), qt.Contains, "  CONSTRAINT users_pkey PRIMARY KEY (id) INCLUDE (covering)")
}
...
`
	c.Assert(legacyPostgresSQL(result), qt.Equals, expected)
}

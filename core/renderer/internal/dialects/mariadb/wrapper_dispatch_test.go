...
// wrapperOwnedRow is one of the four node kinds the MariaDB wrapper answers
...
// mariadbWrapperOwnedKinds is the four node kinds mariadb.Renderer answers with
// a body of its own. Everything else it forwards.
func mariadbWrapperOwnedKinds() []wrapperOwnedRow {
...
// TestMariaDBWrapper_OwnedKindsHaveOneSpelling pins the refusal text and
// measures that both entry points produce it.
//
...
// name, and this test is what keeps that text out of the MariaDB wrapper's
...
// TestMariaDBWrapper_TheShippedSpellingIsTheWrappers measures that the spelling
// kept is the one `ptah schema render` emits.
func TestMariaDBWrapper_OwnedKindsHaveOneSpelling(t *testing.T) {
	for _, row := range mariadbWrapperOwnedKinds() {
		t.Run(row.kind, func(t *testing.T) {
			c := qt.New(t)

			r, err := renderer.NewRenderer(platform.MariaDB)
			...
			renderedSQL, err := renderer.RenderSQL(platform.MariaDB, row.node)
			c.Assert(err, qt.IsNil)
...
// TestMariaDBWrapper_TheShippedSpellingIsTheWrappers measures which spelling a
// user reads.
//
...
func TestMariaDBWrapper_TheShippedSpellingIsTheWrappers(t *testing.T) {
	c := qt.New(t)
	...
	statements, err := renderer.GetOrderedCreateStatements(database, platform.MariaDB)

	c.Assert(err, qt.IsNil)
	c.Assert(statements, qt.DeepEquals, []string{"-- Extension pg_trgm not supported in MariaDB\n"})
}
...
// mariadbPolicyHalves is the two RLS pairs, each split between a wrapper-owned
...
func mariadbPolicyHalves() []policyHalfRow {
	return []policyHalfRow{
		{
			kind:          "CreatePolicyNode",
			node:          ast.NewCreatePolicy("p1", "users"),
			wantOutput:    "-- CREATE POLICY p1 not supported in MariaDB\n",
			wantOmissions: nil,
		},
		{
			kind:       "DropPolicyNode",
			node:       ast.NewDropPolicy("p1", "users"),
			wantOutput: "-- MARIADB: DROP POLICY p1 is not generated for this target; skipped.\n",
			wantOmissions: []renderdiag.Omission{
		...
		},
		{
			kind:          "AlterTableEnableRLSNode",
			node:          ast.NewAlterTableEnableRLS("users"),
			wantOutput:    "-- ALTER TABLE users ENABLE ROW LEVEL SECURITY not supported in MariaDB\n",
			wantOmissions: nil,
		},
		{
			kind:       "AlterTableDisableRLSNode",
			node:       ast.NewAlterTableDisableRLS("users"),
			wantOutput: "-- MARIADB: DISABLE ROW LEVEL SECURITY on users is not generated for this target; skipped.\n",
			wantOmissions: []renderdiag.Omission{
...
// TestMariaDBWrapper_PolicyHalvesReportDifferently pins the asymmetry inside
// each RLS pair, so flattening a pair either way shows up.
//
...
func TestMariaDBWrapper_PolicyHalvesReportDifferently(t *testing.T) {
	for _, row := range mariadbPolicyHalves() {
		t.Run(row.kind, func(t *testing.T) {
			c := qt.New(t)
...
// TestMariaDBWrapper_OwnAndForwardedWritesShareOneBuffer pins that this wrapper
...
func TestMariaDBWrapper_OwnAndForwardedWritesShareOneBuffer(t *testing.T) {
	c := qt.New(t)
	...
	c.Assert(r.Output(), qt.Equals, "CREATE SCHEMA `app`;\n"+
		"-- Extension pg_trgm not supported in MariaDB\n"+
		"DROP TABLE `users`;\n")
}

// TestMariaDBWrapper_ResetClearsWhatTheWrapperWrote is the other half of the
...
func TestMariaDBWrapper_ResetClearsWhatTheWrapperWrote(t *testing.T) {
	c := qt.New(t)

	r := mariadb.New()
	c.Assert(ast.NewExtension("pg_trgm").Accept(r), qt.IsNil)
	c.Assert(r.Output(), qt.Equals, "-- Extension pg_trgm not supported in MariaDB\n")

	r.Reset()
	c.Assert(r.Output(), qt.Equals, "")

	c.Assert(ast.NewCreatePolicy("p1", "users").Accept(r), qt.IsNil)
	c.Assert(r.Output(), qt.Equals, "-- CREATE POLICY p1 not supported in MariaDB\n")
}

...
// The MariaDB renderer is a thin wrapper: it holds the shared mysqllike
// renderer in a named field, names four node kinds in its dispatch and hands
// every other kind to that field. A case dropped from the dispatch still
// renders, because the shared renderer answers the kind too -- with its own
// lowercase dialect name, at exit 0, and only in the text.
//
...
// mariadbCensusRow is one node kind and what this target makes of it.
type mariadbCensusRow struct {
...
// mariadbRenderedKinds lists every node kind MariaDB answers without an error,
// with the exact SQL or refusal comment it writes.
//
...
// instead are in mariadbRefusedKinds.
func mariadbRenderedKinds() []mariadbCensusRow {
	return []mariadbCensusRow{
		{
			// MariaDB has sequences, so nothing refuses this node -- and an
			// ALTER SEQUENCE that changes no attribute has nothing to write.
		...
		},
		{
			...
			want: "-- MARIADB: DISABLE ROW LEVEL SECURITY on users is not generated for this target; skipped.\n",
		},
		{
		...
		},
		{
			...
			want: "-- MARIADB does not support ALTER TYPE - type changes are handled through ALTER TABLE MODIFY COLUMN\n",
		},
		{
		...
		{
			// The capability split: one shared body renders here and refuses
			// on MySQL. TestSequenceCapabilitySplit in the mysqllike suite
			// holds the pair.
		...
		},
		{
			...
			want: "-- MARIADB: CREATE TYPE mood is not generated for this target; skipped.\n",
		},
		{
			kind: "CreateViewNode",
		...
		},
		{
			...
			want: "-- MARIADB: DROP POLICY p1 is not generated for this target; skipped.\n",
		},
		{
			kind: "DropRoleNode",
		...
		},
		{
			...
			want: "-- MARIADB: DROP TYPE mood is not generated for this target; skipped.\n",
		},
		{
			kind: "DropViewNode",
		...
		{
			// MariaDB spells an enumeration inline on the column, so a
			// standalone enum node has nothing of its own to write.
			kind: "EnumNode",
...
// mariadbRefusedKinds lists every node kind that aborts a MariaDB render, with
// the message it aborts on. A refusal returns no SQL at all.
func mariadbRefusedKinds() []mariadbCensusRow {
	return []mariadbCensusRow{
		...
		},
		{
			...
			want: "unsupported feature: mariadb: CREATE MATERIALIZED VIEW mv1: materialized views are not " +
				"supported by MySQL or MariaDB; remove matview definitions for this target",
		},
		{
			kind: "DropMaterializedViewNode",
			node: ast.NewDropMaterializedView("mv1"),
			want: "unsupported feature: mariadb: DROP MATERIALIZED VIEW mv1: materialized views are not " +
				"supported by MySQL or MariaDB; remove matview definitions for this target",
		},
		{
			kind: "RefreshMaterializedViewNode",
			node: ast.NewRefreshMaterializedView("mv1"),
			want: "unsupported feature: mariadb: REFRESH MATERIALIZED VIEW mv1: materialized views are not " +
				"supported by MySQL or MariaDB; remove matview definitions for this target",
		},
		{
			kind: "UpsertNode",
			...
			want: "unsupported feature: mariadb: upsert rendering is not implemented",
		},
	}
}

// mariadbCensusTable is the CREATE TABLE fixture, built where a composite
// literal cannot express it.
func mariadbCensusTable() *ast.CreateTableNode {
	table := ast.NewCreateTable("users")
	table.AddColumn(ast.NewColumn("id", "INT"))
	return table
}

// TestMariaDBDispatch_EveryNodeKindCensus_HappyPath pins what each node kind
// renders on MariaDB through the public render path.
//
...
func TestMariaDBDispatch_EveryNodeKindCensus_HappyPath(t *testing.T) {
	for _, row := range mariadbRenderedKinds() {
		t.Run(row.kind, func(t *testing.T) {
			c := qt.New(t)

			sql, err := renderer.RenderSQL(platform.MariaDB, row.node)
...
// TestMariaDBDispatch_EveryNodeKindCensus_FailurePath pins the node kinds
// MariaDB cannot render standalone. Each aborts the render rather than writing
// a skipped comment, and the census records that outcome rather than leaving
// the kind out.
func TestMariaDBDispatch_EveryNodeKindCensus_FailurePath(t *testing.T) {
	for _, row := range mariadbRefusedKinds() {
		t.Run(row.kind, func(t *testing.T) {
			c := qt.New(t)
...
func censusRows() []mariadbCensusRow {
	return append(mariadbRenderedKinds(), mariadbRefusedKinds()...)
}

// TestMariaDBDispatch_TheCensusCoversEveryNodeKind is the floor under the two
// censuses: a table that quietly lost a row reports nothing and reads as
// success.
func TestMariaDBDispatch_TheCensusCoversEveryNodeKind(t *testing.T) {
	c := qt.New(t)
...
// TestMariaDBDispatch_EachCensusRowNamesItsNode keeps the kind label honest.
//
...
// the kind it claims.
func TestMariaDBDispatch_EachCensusRowNamesItsNode(t *testing.T) {

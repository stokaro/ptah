...
// primary key constraint depends on it. ast.ColumnNode.Nullable does not carry
// "a key column is NOT NULL" for the AST, because SQLite does not have that
// rule (stokaro/ptah#1235), so this renderer applies it where the dialect is
// known -- the same branch renderColumn takes on the CREATE TABLE path.
// Measured live on PostgreSQL, whose ALTER path has the identical hole and
// plans an unappliable `DROP NOT NULL` without it; this dialect is guarded by

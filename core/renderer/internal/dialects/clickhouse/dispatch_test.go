...
func TestVisitNode_ARoutineBodyReachesTheOutputVerbatim(t *testing.T) {
	tests := []loweredRoutineRow{
		...
		},
		{
			...
			node: &ast.OpaqueRoutineNode{SQL: "CREATE FUNCTION plus_one AS (x) -> x + 1;"},
			want: "CREATE FUNCTION plus_one AS (x) -> x + 1;\n",
		},
		{
	...
	tests := []refusedNodeRow{
		{
			...
			want:   `invalid schema diff: clickhouse: the AST node is nil`,
		},
		{
			name:   "an alter operation on its own",
			node:   &ast.AddColumnOperation{Column: ast.NewColumn("email", "String")},
			wantIs: ptaherr.ErrInvalidSchemaDiff,
			want: `invalid schema diff: clickhouse: \*ast\.AddColumnOperation renders as part of the ` +
		...
		},
		{
			...
			wantIs: ptaherr.ErrInvalidSchemaDiff,
			want: `invalid schema diff: clickhouse: \*ast\.DropConstraintOperation renders as part of the ` +
				`statement that carries it, not on its own`,
		},
		{
			...
			wantIs: ptaherr.ErrInvalidSchemaDiff,
			want: `invalid schema diff: clickhouse: \*ast\.EnumTypeDef renders as part of the ` +
				`statement that carries it, not on its own`,
		},
		{
			...
			wantIs: ptaherr.ErrInvalidSchemaDiff,
			want: `invalid schema diff: clickhouse: \*ast\.AddEnumValueOperation renders as part of the ` +
				`statement that carries it, not on its own`,
		},
		{
			...
			want:   `unsupported feature: clickhouse: \*clickhouse_test\.unroutedNode has no handler in this renderer`,
		},
	}

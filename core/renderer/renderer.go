...
// resolved from catalog.ServerInfo.Version; NewRenderer remains the offline default.
...
func GetOrderedCreateStatements(desired *schemamodel.Database, dialect string) ([]string, error) {
	return GetOrderedCreateStatementsWithCapabilities(desired, dialect, capability.ForDialect(dialect))
}
...
func ValidateSchema(desired *schemamodel.Database, dialect string) error {
	return ValidateSchemaWithCapabilities(desired, dialect, capability.ForDialect(dialect))
}
...
func ValidateSchemaWithCapabilities(
	...
	_, err := prepareDatabaseForRendering(desired, dialect, caps)
	return err
}
...
func GetOrderedCreateStatementsWithCapabilities(
	...
	caps = capability.WithDeclaredExtensions(caps, declaredExtensionNames(desired))
	database, err := prepareDatabaseForRendering(desired, dialect, caps)
	if err != nil {
		return nil, err
	}
...
// validates, so that package needs no knowledge of schemamodel.

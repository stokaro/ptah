...
func (r *renderer) renderTables() {
	...
	for _, table := range tables {
		r.renderTable(table, fieldsByStruct[table.StructName], indexesByTable[table.QualifiedName()], constraintsByTable[table.QualifiedName()])
	}
}

func (r *renderer) renderTable(table goschema.Table, fields []goschema.Field, indexes []goschema.Index, constraints []goschema.Constraint) {
...
func (r *renderer) reportUnsupportedObjects() {
	r.warnCount("extensions", len(r.db.Extensions), "extensions cannot be represented in the supported Atlas HCL subset")
	r.warnCount("functions", len(r.db.Functions), "functions contain raw SQL bodies and cannot be represented structurally yet")
	r.warnCount("views", len(r.db.Views), "views contain raw SQL bodies and cannot be represented structurally yet")
	r.warnCount("materialized_views", len(r.db.MaterializedViews), "materialized views contain raw SQL bodies and cannot be represented structurally yet")
	r.warnCount("triggers", len(r.db.Triggers), "triggers contain raw SQL bodies and cannot be represented structurally yet")
	r.warnCount("rls_policies", len(r.db.RLSPolicies), "RLS policies cannot be represented in the supported Atlas HCL subset")
	r.warnCount("rls_enabled_tables", len(r.db.RLSEnabledTables), "RLS enablement cannot be represented in the supported Atlas HCL subset")
	r.warnCount("roles", len(r.db.Roles), "roles cannot be represented in the supported Atlas HCL subset")
	r.warnCount("grants", len(r.db.Grants), "grants cannot be represented in the supported Atlas HCL subset")
}
...
func (r *renderer) warnCount(path string, count int, message string) {
	if count == 0 {
		return
	}
	r.warn(path, fmt.Sprintf("%s (%d object(s))", message, count))
}

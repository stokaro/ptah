...
func (p *Parser) handleGenerated(column *ast.ColumnNode) error {
	...
	case "BY":
		...
		return p.handleGeneratedIdentity(column)
	case "ALWAYS":
		p.advance()
	default:
...
func (p *Parser) handleGeneratedIdentity(column *ast.ColumnNode) error {
	if err := p.expect(lexer.TokenIdentifier, "IDENTITY"); err != nil {
		return fmt.Errorf("expected IDENTITY after GENERATED AS: %w", err)
	}
	column.SetAutoIncrement()
	p.skipGeneratedIdentityOptions()
	return nil
}

func (p *Parser) skipGeneratedIdentityOptions() {
	p.skipWhitespace()
	if p.current.Type != lexer.TokenOperator || p.current.Value != "(" {
		return
	}

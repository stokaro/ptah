	...
	return map[string]measuredLine{
		"PostgreSQL 18": {
			...
			carried: map[capability.Capability]string{
				...
				capability.RangeTypes: "this run predates the key and sent no CREATE TYPE ... AS RANGE. PostgreSQL is the engine the key is true FOR; " +
				...
				capability.CatalogPartitions:               "this run asked no partition-catalog question. What decides the key is whether pg_inherits resolves, which every real PostgreSQL has and only a catalog emulation lacks",
				...
				capability.IndexBlocksTableDrop:            "this run predates the key. Measured separately on PostgreSQL 18.4: with `CREATE INDEX ibt_ix ON ibt (id)` in place, `DROP TABLE ibt` is accepted, so no index blocks the drop",
		...
		"MySQL 26": {
			...
			carried: map[capability.Capability]string{
				...
				capability.CatalogPartitions:               "pg_inherits is a PostgreSQL catalog relation, which is not a question this MySQL run could ask",
				...
				capability.IndexBlocksTableDrop:            "this run predates the key. Measured separately on MySQL 26.7.0: with `CREATE INDEX ibt_ix ON ibt (id)` in place, `DROP TABLE ibt` is accepted, so no index blocks the drop",
		...
		"MariaDB 12": {
			...
			carried: map[capability.Capability]string{
				...
				capability.CatalogPartitions:               "pg_inherits is a PostgreSQL catalog relation, which is not a question this MariaDB run could ask",
				...
				capability.IndexBlocksTableDrop:            "this run predates the key. Measured separately on MariaDB 12.3.2: with `CREATE INDEX ibt_ix ON ibt (id)` in place, `DROP TABLE ibt` is accepted, so no index blocks the drop",
